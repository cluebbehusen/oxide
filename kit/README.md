...
- `checkpoint` captures a completed tick boundary: scenario, validated world,
  ...
  independently. `RecordedCheckpoint` carries the existing recorder alongside
  this core so current hosts can continue exporting complete legacy replays.
  Recorder setup and duration are checked; restoration does not re-execute the
  log to prove its correspondence to the world. This internal contract does not
  change ordinary saves, Continue, or recovery.
