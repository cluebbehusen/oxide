...
- `bot_eval` runs the player-facing controller to a decision or tick ceiling,
  ...
  anomaly provenance. It can exchange complete controller configurations between
  seats for paired personality or difficulty comparisons, or compare a
  player-facing profile with the frozen Overseer through an evaluation-only
  command source. Persisted batches are staged and never replace earlier
  evidence. A returned publication error rolls back files created by that
...
controller-wide failure and points replay inspection at the exact unit. The
command-stream hash exposes different seed cells that nevertheless generated
identical play. Treat those metrics as diagnostics; inspect the preserved
replays and use human play and replay judgment to decide whether behavior is
credible or fun.
