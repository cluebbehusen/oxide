...
//!
//! [`serve`] knows only bytes and framing: it takes a handler factory and
//! never mentions the shell, so a windowless session can drive the same
//! loop with its own answering side.

use anyhow::{Context, Result};
use oxide_protocol::{
    MAX_ADVANCE_TICKS, MAX_FRAME_BYTES, Request, RequestEnvelope, ResponseEnvelope,
};
...
/// Ticks per second budgeted for a synchronous advance — the same
/// conservative figure the driver's client uses, so the two sides cannot
/// disagree about what a legal wait is.
const ADVANCE_TICKS_PER_DEADLINE_SECOND: u64 = 1_000;

/// How long a socket thread waits for the frame loop's answer. Past the
...
const REPLY_TIMEOUT: Duration =
    Duration::from_secs(60 + MAX_ADVANCE_TICKS / ADVANCE_TICKS_PER_DEADLINE_SECOND);
...
pub fn spawn(port: u16) -> Result<Receiver<IncomingRequest>> {
    ...
    let (tx, rx) = channel();
    serve(listener, Limits::default(), move || {
        let tx = tx.clone();
        move |envelope: RequestEnvelope| {
            ...
            .ok()?; // main loop is gone; nothing left to serve
            match reply_rx.recv_timeout(REPLY_TIMEOUT) {
                Ok(response) => Some(response),
                Err(RecvTimeoutError::Timeout) => Some(ResponseEnvelope::err(
                    id,
                    format!(
                        "the shell did not answer within {} seconds",
...
fn serve<F, H>(listener: TcpListener, limits: Limits, make_handler: F)
where
    F: Fn() -> H + Send + 'static,
