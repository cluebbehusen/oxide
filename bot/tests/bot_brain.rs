...
mod common;
use common::simulation::{cmd, unit};

use chassis::grid::TilePos;
use oxide_bot::observation::ObservationData;
use oxide_bot::{
    ArmyState, Executive, Intent, Observation, PublicMapBriefing, SeatBot as Brain, Specialty,
};
...
fn arena_sized(width: usize, height: usize, units: Vec<UnitSpec>) -> Scenario {
    Scenario {
        players: common::simulation::players(500),
        ..common::simulation::open_arena(width, height, units)
    }
}
...
fn a_dispatched_defense_that_never_appears_blacklists_only_its_anchor() {
    ...
    let macro_cadence = oxide_bot::difficulty::STRATEGIC_ADMISSION_CADENCE;
...
fn scripted_brain_completes_a_real_raid_and_releases_the_pair_for_reuse() {
    ...
    let profile = oxide_bot::ResolvedProfile::resolve(config);
