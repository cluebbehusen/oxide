...
type CampaignTarget = (
    oxide_sim::ids::PlayerId,
    chassis::grid::TilePos,
    Option<oxide_sim::ids::BuildingId>,
);
...
pub(crate) struct PlanningWork {
    ...
    production: RefCell<crate::allocation::production_work::ProductionWork>,
...
impl PlanningWork {
    ...
    pub(crate) fn infrastructure_sites(
        &self,
        tick: u64,
        kind: oxide_sim::stats::BuildingKind,
    ...
    pub(crate) fn campaign_site_selected(
        &self,
        tick: u64,
    ...
    #[cfg(test)]
    pub(crate) fn with_allowance(allowance: usize) -> Self {
        Self {
            allowance,
    ...
    pub(crate) fn begin(&self, tick: u64) {
    ...
    pub(crate) fn production(
        &self,
        tick: u64,
        capacity: &crate::allocation::AllocationCapacity,
        claims: &crate::allocation::ClaimState,
    ) -> Result<
        Progress<crate::allocation::ResolvedClaimState>,
        crate::allocation::AllocationConflict,
    > {
        self.begin(tick);
        self.production
            .borrow_mut()
    ...
    pub(crate) fn production_forecast(
        &self,
        tick: u64,
        capacity: &crate::allocation::AllocationCapacity,
        claims: &crate::allocation::ClaimState,
    ...
    pub(crate) fn clear_site(&self, kind: oxide_sim::stats::BuildingKind) {
        self.sites.borrow_mut().clear_incumbent(kind);
    }

    pub(crate) fn foundry_indices(&self, tick: u64, count: usize, limit: usize) -> Vec<usize> {
    ...
    pub(crate) fn stats(&self) -> super::observer::PlanningWorkStats {
    ...
    pub(crate) fn candidate_route_field(
        &self,
        query_purpose: QueryPurpose,
        tick: u64,
        grid: super::navigation::KnownGrid<'_>,
    ...
    pub(crate) fn field(
        ...
        query_purpose: QueryPurpose,
        tick: u64,
        map: &crate::PublicMapBriefing,
        blocked: &crate::navigation::public_fields::BlockedGroundLayout,
        sources: impl IntoIterator<Item = chassis::grid::TilePos>,
    ) -> Progress<std::sync::Arc<crate::navigation::public_fields::PublicGroundDistances>> {
        self.begin(tick);
        self.navigation_work(|budget| {
            self.fields
                .borrow_mut()
...
mod tests {
    ...
    fn campaign_site_admission_is_shared_across_calls_and_rotates_past_failures() {
        ...
        let selected: Vec<_> = sites
            ...
            .collect();
        for site in &sites {
            work.campaign_candidate(
                33,
                *site,
                &[(oxide_sim::ids::PlayerId(1), *site, None)],
                |_| Progress::<()>::Deferred,
            );
        }
        assert_eq!(
    ...
    fn campaign_diagnostics_reset_and_site_retention_is_bounded() {
        ...
        work.begin(0);
        let targets = [
            (oxide_sim::ids::PlayerId(1), TilePos::new(10, 10), None),
            (oxide_sim::ids::PlayerId(1), TilePos::new(12, 10), None),
            (oxide_sim::ids::PlayerId(1), TilePos::new(14, 10), None),
        ];
        for x in 0..64 {
    ...
    fn active_field_preparation_outlives_idle_expiry_with_small_slices() {
        ...
        use chassis::grid::TilePos;
        let map = PublicMapBriefing::from_scenario(&oxide_sim::Scenario::skirmish()).unwrap();
    ...
    fn field_preparation_and_site_refinement_share_one_allowance() {
        use crate::{PublicMapBriefing, navigation::public_fields::BlockedGroundLayout};
        use chassis::grid::TilePos;
