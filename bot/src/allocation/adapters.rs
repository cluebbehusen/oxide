...
pub(crate) enum DomainPayload {
    ...
    Support(crate::utility::RepairAssignment),
    ...
    Reconnaissance(crate::utility::ReconProposal),
}
...
pub(crate) fn reconnaissance_investment_proposal(
    proposal: crate::utility::ReconProposal,
...
pub(crate) fn standing_force_investment_proposal(
    ...
    let claims = if let Some((through, current_scrap, forecast_scrap)) = proposal.accumulation() {
        if proposal.reason() == crate::standing_force::StandingForceReason::WoundedSupport {
            ClaimBundle::new(
                current_scrap,
                vec![ForecastClaim {
                    through,
...
impl AcceptedDomainPayloads {
    pub(crate) fn take_support_deployment(&mut self) -> Option<crate::utility::SupportDeployment> {
    ...
    pub(crate) fn take_support(&mut self) -> Option<crate::utility::RepairAssignment> {
        self.support.take()
    }
...
mod tests {
    ...
    use crate::profile::{PersonalityTraits, Specialty};
    use crate::resources::{
    ...
    fn connected_jobs_keep_domain_order_and_are_charged_exactly_once() {
        ...
        let job = crate::strategy::ConnectedProviderJob::fixture(
            kind,
            NOW,
            DEADLINE,
            vec![BuildingId(8)],
