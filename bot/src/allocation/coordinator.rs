...
impl From<crate::resources::PlanningProjectionError> for CoordinatorInputError {
    fn from(error: crate::resources::PlanningProjectionError) -> Self {
    ...
    refine_standing_waits(
        ...
        &mut super::Refinement {
            ...
            production: &mut |capacity, claims| {
                claims
                    .resolve(capacity)
                    .map(crate::planning::Progress::Ready)
            },
        },
    );
}
...
fn deferred_foundation_forecast_claim(
    forecast: &crate::resources::ResourcePlanningProjection,
...
mod tests {
    ...
    fn observation() -> Observation {
        Observation::from_data(ObservationData {
            version: crate::observation::OBSERVATION_VERSION,
            me: PlayerId(0),
            map_width: 20,
            map_height: 20,
    ...
    fn harvester(id: u32) -> UnitObs {
        crate::test_support::unit(id, PlayerId(0), UnitKind::Harvester, TilePos::new(2, 2))
    }
                ...
                producers: producers
                    ...
                    .map(|(id, trainable)| {
                        ProducerPlanningProjection::fixture(
                            ...
                            12,
                            120,
                            vec![120; oxide_sim::stats::QUEUE_CAP],
                            trainable,
                        )
    ...
    fn settlement_trace_records_the_final_foundry_funding_split_once() {
        ...
        let resources = ResourcePlanningProjection::fixture(ResourcePlanningFixture {
            ...
            units: vec![builder],
            builders: vec![crate::resources::BuilderResource {
                id: builder,
                kind: UnitKind::Harvester,
                obligation: None,
            }],
    ...
    fn unpaid_foundation_claims_current_bank_then_earliest_needed_income() {
        ...
        let horizon = obs
            .tick
            .saturating_add(Tick::from(cost).saturating_mul(oxide_sim::stats::RECLAIMER_PERIOD))
            .saturating_add(24);
    ...
    fn unfunded_foundation_owns_all_bounded_income_before_fresh_connected_work() {
        ...
        let connected = FreshConnectedProposal::fixture(FreshConnectedProposalFixture {
            ...
            minimum_claims: ConnectedOffenseClaims::fixture(
                Vec::new(),
                vec![crate::strategy::ConnectedProviderJob::fixture(
                    UnitKind::Harvester,
                    obs.tick,
                    horizon,
                    vec![BuildingId(9)],
    ...
    fn connected_schedule_retains_request_ordinals_across_busy_and_idle_lanes() {
        ...
        let proposal = FreshConnectedProposal::fixture(FreshConnectedProposalFixture {
            ...
            minimum_claims: ConnectedOffenseClaims::fixture(
                Vec::new(),
                vec![
                    crate::strategy::ConnectedProviderJob::fixture(kind, 120, deadline, vec![busy]),
                    crate::strategy::ConnectedProviderJob::fixture(kind, 120, deadline, vec![idle]),
                ],
            ),
            marginal_additions: Vec::new(),
            protected_current_scrap: 0,
