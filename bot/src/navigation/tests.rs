...
fn blocked_starts_overlap_invalid_endpoints_and_corner_rules_match_astar() {
    ...
    for (starts, goals, expected) in [
        ...
        assert_eq!(
            queries
                .between_sets(QueryPurpose::NavigationTest, grid, None, &starts, &goals)
                .available_cost(),
            expected
        );
...
fn search_limit_remains_distinct_from_disconnection_and_partial_success() {
    ...
    assert_eq!(
        queries.search(QueryPurpose::NavigationTest, grid, &query, 0, &mut scratch),
        CostResult::SearchLimit
    );
    assert_eq!(
        queries.search(QueryPurpose::NavigationTest, grid, &query, 9, &mut scratch),
        CostResult::Exact(28)
    );
    ...
    let query = Query::new(grid, None, &[TilePos::new(0, 0)], &[TilePos::new(2, 2)]);
    assert_eq!(
        queries.search(QueryPurpose::NavigationTest, grid, &query, 8, &mut scratch),
    ...
    assert_eq!(
        queries.search(QueryPurpose::NavigationTest, grid, &query, 2, &mut scratch),
...
fn cached_disconnection_is_invalidated_when_a_tile_opens() {
    ...
    assert_eq!(
        queries.between_sets(QueryPurpose::NavigationTest, grid, None, &starts, &goals),
        CostResult::Unreachable
    );
    assert_eq!(
        queries.between_sets(QueryPurpose::NavigationTest, grid, None, &starts, &goals),
...
fn eviction_and_oversized_grids_preserve_answers_without_unbounded_retention() {
    ...
    assert_eq!(
        queries.between_sets(QueryPurpose::NavigationTest, grid, None, &starts, &starts),
        CostResult::Exact(0)
    );
...
fn normalized_sets_reuse_answers_but_opened_tiles_overlays_and_dimensions_do_not() {
    ...
    let first = queries.between_sets(QueryPurpose::NavigationTest, grid, None, &starts, &goals);
    assert_eq!(
        queries.between_sets(
            QueryPurpose::NavigationTest,
            grid,
