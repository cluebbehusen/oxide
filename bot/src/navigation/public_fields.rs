...
impl PublicFieldWork {
    ...
    pub(crate) fn advance(
        ...
        budget: &mut crate::planning::WorkBudget,
    ) -> crate::planning::Progress<Arc<PublicGroundDistances>> {
...
impl BlockedGroundLayout {
    ...
    pub(crate) fn contains(&self, tile: TilePos) -> bool {
...
pub(crate) struct PublicRouteBuilds {
    pub(crate) danger_aware: usize,
...
impl PublicRoutes {
    ...
    #[cfg(test)]
    pub(crate) fn danger_aware_fields(
        &mut self,
        query_purpose: QueryPurpose,
        public_map: &PublicMapBriefing,
        blocked: BlockedGroundLayout,
    ...
    pub(crate) fn danger_aware_source_set(
        &mut self,
        query_purpose: QueryPurpose,
        public_map: &PublicMapBriefing,
        blocked: &BlockedGroundLayout,
    ...
    pub(crate) fn start_fields(
        &mut self,
        query_purpose: QueryPurpose,
        public_map: &PublicMapBriefing,
        starts: &[crate::StartingFoundry],
    ...
    #[cfg(test)]
    pub(crate) fn build_count(&self) -> PublicRouteBuilds {
        self.builds
    }

    #[cfg(test)]
    pub(crate) fn retained_field_counts(&self) -> (usize, usize, usize) {
        (
...
impl PublicGroundDistances {
    ...
    pub(crate) fn from_sources_avoiding(
        query_purpose: QueryPurpose,
        public_map: &PublicMapBriefing,
        sources: impl IntoIterator<Item = TilePos>,
        mut blocked: impl FnMut(TilePos) -> bool,
        ...
        let result = work.advance(
            query_purpose,
            &mut crate::planning::WorkBudget::new(usize::MAX),
        );
        debug_assert_eq!(result, crate::planning::Progress::Ready(()));
    ...
    #[cfg(test)]
    pub(crate) fn visit_footprint_distances(
        &self,
        size: (i32, i32),
    ...
    pub(crate) fn footprint_distance(&self, anchor: TilePos, size: (i32, i32)) -> Option<u32> {
        (0..size.1)
...
pub(crate) struct WorkRoutes<'a, 'b> {
...
mod tests {
        ...
        for changed in [false, true] {
            if changed {
                map.non_ground_terrain = (1..8)
                    .map(|y| (TilePos::new(5, y), oxide_sim::map::Terrain::Pit))
                    .collect();
            }
            let blocked =
    ...
    fn dynamic_danger_never_invalidates_terrain_only_threat_or_start_fields() {
        let starts = vec![
            crate::StartingFoundry {
                player: oxide_sim::ids::PlayerId(0),
                anchor: TilePos::new(1, 1),
            },
            crate::StartingFoundry {
                player: oxide_sim::ids::PlayerId(1),
