...
pub(crate) fn with_build_routes<T>(run: impl FnOnce(&RefCell<PathQueries>) -> T) -> T {
...
pub(crate) struct PathQueries {
    pub(crate) costs: crate::navigation::CostQueries,
...
impl PathBoard<'_> {
    ...
    fn path_prepared(
        ...
        crate::query_work::record(
            self.query_purpose,
            crate::query_work::QueryOperation::PathRequest,
            1,
        );
            ...
            .and_then(|g| g.paths.get(&key))
        {
            crate::query_work::record(
                self.query_purpose,
                crate::query_work::QueryOperation::CacheHit,
                1,
            );
            #[cfg(test)]
            super::work::record(|work| {
        ...
        let eligible = self.grid.open(start, overlay)
            ...
            && self.grid.blocked.len() <= oxide_sim::stats::PATH_EXPANSION_CAP as usize;
    ...
    fn prepare_endpoint_batch(
        ...
        if queries >= 8
            && cells <= oxide_sim::stats::PATH_EXPANSION_CAP as usize
...
fn distance_field(
    ...
    let mut budget = crate::planning::WorkBudget::new(usize::MAX);
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct BaselineEndpointRoute {
    path: Option<Vec<TilePos>>,
    exhausted: bool,
}

pub(crate) type EndpointRoutes = BTreeMap<(CacheClass, TilePos, TilePos), BaselineEndpointRoute>;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct PendingRoute;

pub(crate) struct CandidatePaths<'a> {
    board: PathBoard<'a>,
    overlay: Option<BlockedRect>,
    paths: BTreeMap<(TilePos, TilePos), Vec<TilePos>>,
    planning: Option<(u64, &'a crate::planning::PlanningWork)>,
}
...
impl<'a> CandidatePaths<'a> {
    ...
    pub fn with_planning(mut self, tick: u64, planning: &'a crate::planning::PlanningWork) -> Self {
...
pub(crate) fn path_cost(path: &[TilePos]) -> u32 {
    path.windows(2)
        .map(|pair| {
...
pub(crate) fn shortest_path_between(
    board: PathBoard<'_>,
    starts: &[TilePos],
    goals: &[TilePos],
