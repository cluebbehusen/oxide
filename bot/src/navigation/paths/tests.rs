use super::*;
use crate::navigation::octile as octile_cost;
...
fn endpoint_batches_use_refined_distances_before_building_more_paths() {
    ...
    let (actual, work) =
        crate::navigation::work::measure(|| shortest_path_between(board, &[start], &goals, None));
...
fn repeated_long_routes_share_candidate_fields_and_reduce_total_work() {
    ...
    let (expected, before) = crate::navigation::work::measure(|| {
        starts
            .iter()
            .map(|&start| {
    ...
    let (actual, after) = crate::navigation::work::measure(|| {
        starts
            .iter()
...
fn budgeted_candidates_resume_with_exact_paths_independent_of_observational_cache() {
    ...
    for tick in (0..1_200).step_by(12) {
        ...
        let (actual, work) = crate::navigation::work::measure(|| {
...
fn endpoint_pruning_does_not_build_fields_for_one_off_routes() {
    ...
    let (route, work) =
        crate::navigation::work::measure(|| shortest_path_between(board, &starts, &goals, None));
    assert_eq!(
        route.unwrap(),
        (
            starts[0],
...
fn normal_fields_are_promoted_after_repeated_expensive_routes() {
    ...
    let goal = TilePos::new(4, 40);
    let starts = (20..50).map(|y| TilePos::new(70, y)).collect::<Vec<_>>();
    let (expected, reference_work) = crate::navigation::work::measure(|| {
        starts
            .iter()
            .map(|&start| {
                Search::default().path(QueryPurpose::NavigationTest, 80, 60, start, goal, |tile| {
...
fn repeated_doorstep_searches_share_an_earned_origin_field() {
    ...
    let (reference, exhaustive) = crate::navigation::work::measure(|| {
        goals
            .iter()
            .filter_map(|&goal| {
                Search::default()
    ...
    let (selected, actual) = crate::navigation::work::measure(|| {
