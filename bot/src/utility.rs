...
use super::resources::{
    BuilderLease, FoundationCancellations, ProducerLaneReservations, ResourceSnapshot,
...
pub(crate) use support_allocation::SupportWorkSnapshot;
...
#[cfg(test)]
pub(crate) use construction::AdjudicatedFoundryCommit;
...
#[cfg(test)]
pub(crate) use defensive_investment::DefenseConstruction;
pub(crate) use defensive_investment::FreshDefenseProposal;
pub use economic_investment::EconomicInvestmentKey;
pub(crate) use economic_investment::{
    AirCapacityDemand, EconomicInvestment, EconomicInvestmentContext,
};
pub(crate) use production::{CombatCoreStatus, combat_core_status};
...
const CONTESTED_HARVEST_RADIUS: i32 = oxide_sim::stats::HARVEST_INCIDENT_DANGER_RADIUS;
/// Every eligible contested-region scout has enough vision to cover this
/// complete square from the exact incident tile.
const CONTESTED_RECON_RADIUS: i32 = oxide_sim::stats::HARVEST_INCIDENT_DANGER_RADIUS;
...
const CONTESTED_RECON_SWEEP_TICKS: u64 = oxide_sim::stats::HARVEST_INCIDENT_MEMORY_TICKS;
...
struct ConstructionClaims<'a> {
    cancellations: FoundationCancellations<'a>,
    enlisted: &'a [UnitId],
    reserved: &'a [UnitId],
}
...
impl<'a> ConstructionContext<'a> {
    ...
    const fn during_opening_core(mut self) -> Self {
...
fn expansion_economy(
    ...
    expansion::ExpansionEconomy {
        ...
        ticks_per_minute: u64::from(oxide_sim::TICKS_PER_SECOND).saturating_mul(60),
        now: obs.tick,
...
pub(crate) struct PolicyState {
    ...
    pub(crate) standing_saving: Option<super::standing_force::StandingForceCommitment>,
...
#[derive(Clone, Copy)]
pub(crate) struct DecisionEvidence<'a> {
    pub(crate) battlefield: &'a super::battlefield::BattlefieldAssessment,
    pub(crate) experience: &'a super::experience::Experience,
}

#[cfg(test)]
impl Default for DecisionEvidence<'_> {
    fn default() -> Self {
        static BATTLEFIELD: std::sync::LazyLock<super::battlefield::BattlefieldAssessment> =
            std::sync::LazyLock::new(Default::default);
        static EXPERIENCE: std::sync::LazyLock<super::experience::Experience> =
            std::sync::LazyLock::new(Default::default);
        Self {
...
/// Foundry capital and channel ownership already settled by allocation.
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub(crate) struct FoundryHandoff {
    protected_scrap: u32,
    committed: bool,
    current_scrap: u32,
}
...
impl FoundryHandoff {
    fn spending_after(self, available: u32) -> u32 {
        available.saturating_sub(self.protected_scrap)
    }
}
...
struct PolicyMode<'a> {
    ...
    ground_missions: Option<GroundMissionInputs<'a>>,
...
impl<'a> StrategicUtilityContext<'a> {
    pub(super) fn with_ground_missions(mut self, missions: GroundMissionInputs<'a>) -> Self {
        self.ground_missions = Some(missions);
        self
    }
    ...
    pub(crate) const fn with_foundry_handoff(mut self, foundry: FoundryHandoff) -> Self {
    ...
    pub(crate) const fn with_producer_lane_reservations(
        mut self,
        reservations: &'a ProducerLaneReservations,
    ) -> Self {
        self.producer_lane_reservations = reservations;
...
impl UtilityPolicy {
    ...
    fn post_floor_deferred_claims(
        ...
        for claim @ (kind, anchor) in claims {
            if self.state.economic_foundation.as_ref().is_some_and(|plan| {
                plan.build()
        ...
        let mut available = exceptions.iter().fold(admission_scrap, |scrap, (kind, _)| {
    ...
    fn projected_foundries(obs: &Observation) -> (Vec<TilePos>, usize) {
        Self::projected_foundries_after(obs, FoundationCancellations::default())
    }

    fn projected_foundries_after(
        obs: &Observation,
        ...
        self.think_inner(
            ...
            ThinkContext {
                ...
                mode: PolicyMode {
                    ...
                    ground_missions: context.ground_missions,
                    admit_voluntary_macro: strategic_admission_tick(obs.tick),
    ...
    fn think_inner(
        ...
        if let Some((builder, kind, anchor)) = self.state.economic_cancelled_founder.take()
            && obs
                .my_units
                .iter()
        ...
        let utility_admission_scrap = obs.scrap.saturating_sub(prior_scrap_commitment);

        if utility_admission_scrap > self.state.bank_seen || obs.tick == 0 {
            self.state.bank_grew_at = obs.tick;
        }
        self.state.bank_seen = utility_admission_scrap;
        ...
        self.state.desperate = obs.tick.saturating_sub(self.state.bank_grew_at) > 1_600;
        ...
        let cancelled: Vec<_> = obs
            .my_units
            .iter()
            .filter_map(|unit| unit.founding)
            .filter(|claim| !retained_deferred_claims.contains(claim))
            .collect();
        let cancellations = FoundationCancellations(&cancelled);
        let mut foundry = foundry;
        let deferred_scrap = Self::deferred_claims_commitment(&retained_deferred_claims);
    ...
    fn refresh_contested_harvest_regions(
        ...
        let worker_incidents: Vec<_> = obs
            ...
            .collect();
        self.state.harvester_watch = current_harvesters;
        ...
        let danger = (!self.state.contested_harvest_regions.is_empty())
        ...
        for region in &mut self.state.contested_harvest_regions {
            ...
            let currently_dangerous = danger.contains_with_margin(
                region.center,
                oxide_sim::stats::HARVEST_INCIDENT_DANGER_RADIUS,
            );
    ...
    fn harvest_location_contested(&self, location: TilePos) -> bool {
        Self::location_in_contested_regions(&self.state.contested_harvest_regions, location)
    }
    ...
    fn evacuate_contested_workers(
        ...
        let needs_danger = !self.state.evacuating_workers.is_empty()
            || obs
                .my_units
                .iter()
        ...
        for unit in obs.my_units.iter().filter(|unit| {
            ...
            if (!self.state.evacuating_workers.contains(&unit.id) || unit.idle)
        ...
        self.state.evacuating_workers.sort_unstable();
        self.state.evacuating_workers.dedup();
    ...
    fn worker_evacuation_goal(
        ...
        for radius in 0..=max_radius {
            ...
            crate::query_work::record(
                QueryPurpose::EvacuationDestination,
                crate::query_work::QueryOperation::EvacuationCandidates,
...
mod tests {
    ...
    fn standing_force_escape_requires_proven_static_disconnection() {
        ...
        obs.enemy_buildings.push(crate::test_support::building(
            20,
            PlayerId(1),
    ...
    fn a_deficient_core_keeps_only_a_safe_unoccupied_starting_home_extractor_claim() {
        ...
        contested_policy.state.contested_harvest_regions = vec![ContestedHarvestRegion {
            center: frame,
            last_evidence: obs.tick,
            sweep_started_at: None,
        }];
    ...
    #[test]
    fn shared_founders_reserve_one_construction_cost_per_site() {
        ...
        let claims = UtilityPolicy::deferred_claims(&obs);
        assert_eq!(claims.len(), 2);
        assert_eq!(
            UtilityPolicy::deferred_claims_commitment(&claims),
            expected_deferred
        );
        let handoff = FoundryHandoff {
            protected_scrap: 300,
            committed: true,
            current_scrap: 0,
        };
        assert_eq!(
            handoff.spending_after(obs.scrap - 70 - expected_deferred),
            obs.scrap - 70 - expected_deferred - 300
        );
        assert_eq!(handoff.spending_after(10), 0);
    }
    ...
    fn starvation_clock_tracks_the_fixed_utility_admission_bank() {
        ...
        assert_eq!(policy.state.bank_seen, 50);
    ...
    fn typed_capital_ownership_blocks_residual_paid_sustain() {
        let home = TilePos::new(3, 3);
        let tender =
            crate::test_support::unit(1, PlayerId(0), UnitKind::Tender, TilePos::new(5, 5));
        let patient = UnitObs {
            hp: UnitKind::Sentinel.stats().max_hp / 4,
            ..crate::test_support::unit(2, PlayerId(0), UnitKind::Sentinel, TilePos::new(6, 5))
        };
        ...
        funded_policy.admit_test_repair(
            ...
            &map,
            UnitId(1),
            oxide_sim::ids::Target::Unit(UnitId(2)),
            &mut funded_prelude,
        );
        let available = funded_policy.think_with_intelligence(
            ...
            &[],
            &[],
            StrategicUtilityContext::new(&[], &[], &[], &map, funded_prelude, Default::default()),
        );
    ...
    fn strategic_utility_does_not_rerank_an_adjudicated_emergency_defense() {
        ...
        assert!(
            ...
            "the residual facade cannot originate current-threat emergency work: {residual:?}"
        );
        ...
        let intents = policy.think_with_intelligence(
            ...
            &[],
            &[],
            StrategicUtilityContext::new(&[], &[], &[], &map, prelude, Default::default())
    ...
    fn reserved_founder_keeps_ownership_after_becoming_a_paid_builder() {
        ...
        assert_eq!(
            resources.builders()[0].obligation,
            Some(crate::resources::BuilderObligation::Build(BuildingId(23)))
        );
        assert!(!builder_is_free(&obs, &obs.my_units[0]));
    }
    ...
    fn legacy_air_harass_waits_for_the_projected_opening_core() {
        ...
        let wing_kind = oxide_sim::stats::Role::AirGround.unit_for(oxide_sim::Faction::Ferrous);
    ...
    fn fighter(id: u32, player: PlayerId, tile: TilePos) -> UnitObs {
        crate::test_support::unit(id, player, UnitKind::Sentinel, tile)
    }
    ...
    fn build_route_is_safe(
        ...
        crate::navigation::commands::build_command_path_avoids(
            obs,
            unit,
            anchor,
    ...
    fn distinct_incidents_stay_canonical_and_exact_repeats_renew_through_policy_think() {
        ...
        assert!(
            left.chebyshev(right) > oxide_sim::stats::HARVEST_INCIDENT_DANGER_RADIUS,
        ...
        for initial in [vec![right, left], vec![left, right]] {
            ...
            assert_eq!(policy.state.contested_harvest_regions.len(), 2);
            ...
            snapshots.push(policy.state.contested_harvest_regions.clone());
        }
    ...
    fn overlapping_incidents_preserve_their_full_union_after_warnings_expire() {
        ...
        assert!(policy.state.contested_harvest_regions.is_empty());
        assert!(!policy.harvest_location_contested(second));
        assert!(!policy.harvest_location_contested(second_edge));
    }
    ...
    fn contested_region_cap_evicts_oldest_evidence_with_position_ties() {
        ...
        let mut policy = UtilityPolicy::new();
        policy.state.contested_harvest_regions = vec![
            ContestedHarvestRegion {
        ...
        policy.state.contested_harvest_regions.extend(
            (0..oxide_sim::stats::HARVEST_INCIDENT_CAP - 1).map(|index| ContestedHarvestRegion {
        ...
        assert_eq!(
            policy.state.contested_harvest_regions.len(),
            oxide_sim::stats::HARVEST_INCIDENT_CAP + 2
        );
        ...
        let centers: Vec<_> = policy
            ...
            .map(|region| region.center)
            .collect();
        assert_eq!(centers.len(), oxide_sim::stats::HARVEST_INCIDENT_CAP);
    ...
    fn active_air_capacity_owns_the_sole_builder_before_frame_restoration() {
        ...
        let work = [AirCapacityDemand {
            ...
            service: crate::allocation::StandingForceServiceKey::point(home),
        }];
        let quote = policy
    ...
    #[test]
    fn restoration_preempts_harvest_but_respects_same_decision_stops() {
        for founding in [None, Some((BuildingKind::Fabricator, TilePos::new(17, 3)))] {
            let home = TilePos::new(3, 8);
            let frame = home.offset(8, 0);
            let source = home.offset(2, 4);
            let mut worker = harvester(1, founding);
            worker.tile = home.offset(1, 4);
            worker.idle = founding.is_none();
            ...
            assert_eq!(obs.my_units[0].founding, founding);
            if founding.is_some() {
                ...
                assert!(
                    ...
                    "a stopped founder stays reserved for this decision: {intents:?}"
                );
                continue;
            }
            assert_eq!(
                intents.contains(&Intent::AssignHarvest {
                    unit: UnitId(1),
                    node: source,
                }),
                founding.is_none()
            ...
            assert!(matches!(
                intents.iter().find(|intent| !matches!(intent, Intent::StopUnits { .. })),
    ...
    fn an_unfinished_foundry_site_keeps_orphan_relief_alive() {
        ...
        obs.my_buildings.push(BuildingObs {
            hp: 1,
            built: false,
            ..crate::test_support::building(7, PlayerId(0), BuildingKind::Foundry, anchor)
        });
        obs.my_queues.push(Vec::new());
    ...
    fn an_automatic_upgrade_is_not_an_orphaned_site() {
        ...
        obs.my_buildings.push(BuildingObs {
            ...
            built: false,
            tier: 1,
            ..crate::test_support::building(7, PlayerId(0), BuildingKind::Turret, anchor)
        });
        obs.my_queues.push(Vec::new());
        let mut policy = UtilityPolicy::new();
        let mut budget = 0;
    ...
    fn tender_support_continues_while_harvesters_honor_a_construction_promise() {
        ...
        policy.admit_test_repair(
            ...
            &public_map(&initial),
            UnitId(3),
            oxide_sim::ids::Target::Unit(UnitId(5)),
            &mut Vec::new(),
        );
    ...
    fn building_repair_is_one_persistent_program() {
        ...
        crate::test_support::edit_building(&mut state, foundry, |building| {
            building.hp = BuildingKind::Foundry.base_stats().max_hp / 2
        });
    ...
    fn foundry_commitment_counts_distinct_unpaid_sites_not_crewmates() {
        ...
        obs.my_buildings.push(BuildingObs {
            built: false,
            ..crate::test_support::building(7, PlayerId(0), BuildingKind::Foundry, already_paid)
        });
        obs.my_queues.push(Vec::new());
    ...
    fn resolved_greed_reaches_the_expansion_appetite_without_changing_capability() {
        let high_profile = crate::profile::ResolvedProfile::resolve(BotConfig::scripted(
            BotDifficulty::Standard,
            BotStance::Balanced,
            1_616_304,
        ));
        let low_profile = crate::profile::ResolvedProfile::resolve(BotConfig::scripted(
            BotDifficulty::Standard,
            BotStance::Balanced,
            1_616_305,
        ));
    ...
    fn difficulty_changes_attention_and_risk_without_redealing_composition() {
        let prime_profile = crate::profile::ResolvedProfile::resolve(BotConfig::scripted(
            BotDifficulty::Prime,
            BotStance::Balanced,
