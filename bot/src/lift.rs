...
impl LiftProducerTiming {
    ...
    pub(crate) const fn enqueued_at(self) -> Tick {
        self.enqueued_at
    }

    pub(crate) const fn starts_at(self) -> Tick {
        self.starts_at
    }

    pub(crate) const fn ready_at(self) -> Tick {
        self.ready_at
    }
...
impl LiftProducerFunding {
    pub(crate) const fn new(current_scrap: u32, forecast_scrap: u32) -> Self {
        Self {
            current_scrap,
            forecast_scrap,
        }
...
impl LiftProducerAssignment {
    ...
    pub(crate) const fn timing(self) -> LiftProducerTiming {
        self.timing
    }
}
...
pub(crate) struct ActiveLiftProductionObligation {
    accepted_at: Tick,
    deadline: Tick,
...
impl ActiveLiftProductionObligation {
    ...
    pub(crate) fn producer_jobs(&self) -> &[LiftProducerAssignment] {
        &self.producer_jobs
    }

    #[cfg(test)]
    pub(crate) fn producer_schedule_is_executable(
        &self,
...
impl LiftPlanner {
    ...
    /// allocation pass.
    pub(crate) fn active_production_obligation(&self) -> Option<ActiveLiftProductionObligation> {
        let operation = self
            .operation
            .as_ref()
    ...
    pub(crate) fn issued_production_assignments(
        &self,
        observed_at: Tick,
    ...
    pub(crate) fn next_producer_request_ordinal(&self) -> Option<usize> {
        self.operation
            .as_ref()
            .filter(|operation| operation.phase == LiftPhase::Provision)
    ...
    pub(crate) fn mark_producers_issued(&mut self, ordinals: &[usize]) {
    ...
    pub(crate) fn recover_invalid_production(&mut self, observed_at: Tick) {
...
mod tests {
    ...
    fn a_stranded_carrier_tries_distinct_drops_then_bounds_its_recovery() {
        ...
        obs.my_carried_units = (1..=3)
            .map(|id| crate::observation::CarriedUnitObs {
                carrier: UnitId(900),
                id: UnitId(id),
        ...
        assert_eq!(report.outcome, crate::experience::Outcome::Aborted);
        assert_eq!(
            report.reason,
            crate::experience::OutcomeReason::BlockedRoute
        );
