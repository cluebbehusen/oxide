...
use oxide_sim::stats::{BuildingKind, QUEUE_CAP, UnitKind};
...
pub(crate) struct FoundationCancellations<'a>(pub(crate) &'a [(BuildingKind, TilePos)]);

impl FoundationCancellations<'_> {
    pub(crate) fn retained(self, unit: &UnitObs) -> Option<(BuildingKind, TilePos)> {
        unit.founding.filter(|claim| !self.0.contains(claim))
    }

    pub(crate) fn builder_is_free(self, obs: &Observation, unit: &UnitObs) -> bool {
    ...
    fn sequence_timing(&self, planned: &[UnitKind]) -> Option<ProductionTiming> {
        ...
        let current_egress = planned.last().map_or(ProducerEgress::NotRequired, |kind| {
            if self.kind == BuildingKind::Airworks
                && kind.stats().domain == oxide_sim::stats::Domain::Air
            {
        ...
        .filter_map(|building| {
            let (kind, amount, period, first_payment_tick) = match building.kind {
                BuildingKind::Reclaimer if building.tier == 0 => (
                    ...
                    next_multiple_at_or_after(obs.tick, oxide_sim::stats::RECLAIMER_PERIOD)?,
                ),
                BuildingKind::Reclaimer => (
                    RecurringIncomeKind::Refinery,
                    1,
                    oxide_sim::stats::REFINERY_PERIOD,
                    next_multiple_at_or_after(obs.tick, oxide_sim::stats::REFINERY_PERIOD)?,
                ),
                BuildingKind::Foundry => {
                    let completed_tick = next_multiple_at_or_after(
                        obs.tick
                            .checked_add(1)?
                            .max(oxide_sim::stats::FOUNDRY_DRIP_START_TICK),
                ...
                BuildingKind::Extractor => {
                    ...
                    let (kind, (amount, period)) = if supported {
                        (
                            ...
                            oxide_sim::stats::EXTRACTOR_SUPPORTED_YIELD,
                        )
                    } else {
                        (
                            RecurringIncomeKind::RemoteExtractor,
                            oxide_sim::stats::EXTRACTOR_REMOTE_YIELD,
                        )
                    };
                    let completed_tick =
