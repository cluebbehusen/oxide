...
pub(crate) enum ProposalKey {
    ...
    Economy(crate::utility::EconomicInvestmentKey),
    /// Exact repair work with one worker and one patient.
    Support(crate::utility::SupportKey),
    /// A repair provider for exact route-local wounded demand.
    SupportProcurement(StandingForceKey),
    /// An exact repair-service foundation.
    SupportConstruction(crate::utility::EconomicInvestmentKey),
    /// Exact allied Foundry and frozen relief membership.
    SupportRelief(BuildingId),
    /// Exact screen or anti-air service for an exposed asset.
    SupportDeployment(crate::utility::ProtectionKey),
    /// One exact observer for an independently useful information question.
    Reconnaissance(crate::utility::ReconProposalKey),
}

/// Canonical set of individually legal builds that cannot share one layout.
...
impl IncompatibleLayoutSet {
    ...
    pub(crate) fn new(first: ProposalKey, second: ProposalKey) -> Option<Self> {
        Self::from_keys(vec![first, second])
    }

    pub(crate) fn from_keys(mut keys: Vec<ProposalKey>) -> Option<Self> {
        keys.sort_unstable();
        (keys.len() >= 2 && keys.windows(2).all(|pair| pair[0] != pair[1])).then_some(Self { keys })
    }
    ...
    fn is_selected<Payload>(
        ...
        self.keys
            .iter()
            .all(|key| selected.iter().any(|&index| proposals[index].key() == *key))
    }
}
...
impl ProposalKey {
    const fn domain(self) -> ProposalDomain {
        match self {
            ...
            Self::Support(_) => ProposalDomain::Support,
            Self::SupportProcurement(_) => ProposalDomain::Support,
            Self::SupportConstruction(_) => ProposalDomain::Support,
            Self::SupportRelief(_) => ProposalDomain::Support,
            Self::SupportDeployment(_) => ProposalDomain::Support,
            Self::Reconnaissance(_) => ProposalDomain::Reconnaissance,
        }
    }
}
...
impl AllocationPersonality {
    fn preference(self, proposal: ProposalKey) -> u16 {
        match proposal {
            ...
            ProposalKey::Support(_) => self.standing_force,
            ProposalKey::SupportProcurement(_) => self.standing_force,
            ProposalKey::SupportConstruction(_) => self.standing_force,
            ProposalKey::SupportRelief(_) => self.standing_force,
            ProposalKey::SupportDeployment(_) => self.standing_force,
            ProposalKey::Reconnaissance(_) => self.offense,
        }
    }
}
...
pub(crate) struct AllocationCapacity {
    ...
    paid_queue: std::collections::BTreeMap<(BuildingId, UnitKind), usize>,
}
...
impl AllocationCapacity {
    ...
    pub(crate) fn from_snapshot(
        ...
        let mut paid_queue = std::collections::BTreeMap::new();
        for lane in resources.producers() {
            for (kind, _) in lane.queued_readiness() {
                *paid_queue.entry((lane.producer, kind)).or_insert(0usize) += 1;
            }
        }
        Ok(Self {
    ...
    fn fixture(resources: ResourcePlanningProjection) -> Self {
        Self {
            resources,
            buildings: Vec::new(),
            paid_queue: std::collections::BTreeMap::new(),
        }
    }
}
...
pub(crate) struct ClaimBundle {
    ...
    buildings: Vec<BuildingId>,
    paid_queue: Vec<PaidQueueClaim>,
}

impl ClaimBundle {
    pub(crate) fn with_paid_queue(mut self, mut claims: Vec<PaidQueueClaim>) -> Self {
        claims.sort_unstable();
        assert!(
            claims.windows(2).all(|pair| pair[0] != pair[1]),
            "one payload cannot credit a paid occurrence twice"
        );
        self.paid_queue = claims;
        self
    }

    pub(crate) fn paid_queue(&self) -> &[PaidQueueClaim] {
        &self.paid_queue
    }
...
impl<Payload> InvestmentProposal<Payload> {
    ...
    pub(in crate::allocation) const fn fresh(
        key: ProposalKey,
        case: ProposalCase,
        claims: ClaimBundle,
        payload: Payload,
    ...
    pub(in crate::allocation) const fn retained(
        key: ProposalKey,
        case: ProposalCase,
        accepted_at: Tick,
        claims: ClaimBundle,
    ...
    pub(in crate::allocation) const fn with_personality_preference(
        mut self,
    ...
    pub(in crate::allocation) const fn with_domain_preference(
        mut self,
        domain_preference: usize,
    ) -> Self {
    ...
    pub(in crate::allocation) const fn with_voluntary_scrap_guard(mut self, amount: u32) -> Self {
    ...
    pub(in crate::allocation) const fn with_minimum_residual_scrap(mut self, amount: u32) -> Self {
    ...
    /// prefix.
    pub(in crate::allocation) const fn satisfies_voluntary_scrap_guard_within(
        mut self,
        queue_depth: usize,
    ) -> Self {
    ...
    #[cfg(test)]
    pub(in crate::allocation) const fn claims_mut(&mut self) -> &mut ClaimBundle {
        &mut self.claims
    }

    /// Opaque domain payload retained without recomputation.
    pub(in crate::allocation) const fn payload(&self) -> &Payload {
        &self.payload
    }

    /// Mutable domain payload for adapter-owned scale selection.
    pub(in crate::allocation) const fn payload_mut(&mut self) -> &mut Payload {
        &mut self.payload
    }

    /// Splits an accepted proposal at the adapter-owned commit boundary.
    pub(in crate::allocation) fn into_parts(self) -> (ProposalKey, ClaimBundle, Payload) {
...
pub(crate) enum AllocationConflict {
    /// A paid occurrence does not exist in this observation.
    UnknownPaidQueue(PaidQueueClaim),
    /// Two assignments credit the same already-paid occurrence.
    PaidQueue {
        claim: PaidQueueClaim,
        owner: ClaimOwner,
    },
    ...
    IncompatibleLayout {
        /// Canonically ordered identities whose complete layout is unsafe.
        keys: Vec<ProposalKey>,
    },
...
pub(crate) enum ObligationKey {
    ...
    SavedEconomy(crate::utility::EconomicInvestmentKey),
...
impl Ord for ObligationKey {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        if let (Self::Reconnaissance(left), Self::Reconnaissance(right)) = (self, other) {
            return left.cmp(right);
        }
        if let (Self::Support(left), Self::Support(right)) = (self, other) {
            return left.cmp(right);
        }
...
impl<Payload> AllocationResult<Payload> {
    ...
    pub(crate) fn try_extend_connected_offense(
        ...
        let resolved =
            self.refine_connected_offense(capacity, key, claims, &mut |capacity, claims| {
                claims
                    .resolve(capacity)
    ...
    fn refine_connected_offense(
        &mut self,
        capacity: &AllocationCapacity,
        key: ConnectedOffenseKey,
        claims: &ClaimBundle,
        refine: &mut ProductionResolver<'_>,
    ) -> Result<bool, AllocationConflict> {
...
pub(crate) fn allocate_requiring_planned<Payload>(
    capacity: &AllocationCapacity,
    obligations: Vec<ImportedObligation>,
    proposals: Vec<InvestmentProposal<Payload>>,
    personality: AllocationPersonality,
    required: ProposalKey,
    planning: &super::planning::PlanningWork,
) -> Result<Option<AllocationResult<Payload>>, AllocationError> {
    allocate_refined(
        capacity,
        obligations,
...
type ProductionResolver<'a> = dyn FnMut(
        &AllocationCapacity,
        &ClaimState,
...
struct Refinement<'a> {
    layout: &'a mut LayoutValidator<'a>,
...
fn allocate_refined<Payload>(
    ...
    let individual_checks: Vec<_> = proposals
        ...
        .map(|(index, proposal)| {
            ...
            state
                ...
                .and_then(|()| state.validate_production_bounds(capacity))
                .map(|()| {
                    state.producer_jobs.is_empty()
                        && state.deferrable_capital.is_empty()
                        && state.voluntary_scrap_guard.satisfier.is_none()
                })
        })
        .collect();
    ...
    let mut refinements: BTreeMap<Vec<usize>, Result<(), AllocationConflict>> = individual_checks
        .iter()
        .enumerate()
        .filter_map(|(index, result)| match result {
            Ok(true) => Some((vec![index], Ok(()))),
            Err(conflict) => Some((vec![index], Err(conflict.clone()))),
            Ok(false) => None,
        })
        .collect();
    let mut evaluate = |selected: &[usize]| {
        ...
        let keys = selected
            ...
            .collect::<Vec<_>>();
        if let Some(conflict) = (refinement.layout)(&keys) {
    ...
    let decisions = proposals
        ...
        .map(|(index, proposal)| {
            ...
            } else if let Some(conflict) = individual_checks[index].as_ref().err().or_else(|| {
                refinements
                    .get(&vec![index])
...
pub(crate) struct ClaimState {
    ...
    paid_queue: std::collections::BTreeMap<PaidQueueClaim, ClaimOwner>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct ResolvedClaimState {
...
impl ClaimState {
    ...
    fn stage(
        ...
        for &claim in &claims.paid_queue {
            let count = capacity
                .paid_queue
                .get(&(claim.producer, claim.kind))
                .copied()
                .unwrap_or(0);
            if claim.occurrence >= count {
                return Err(AllocationConflict::UnknownPaidQueue(claim));
            }
            if let Some(&owner) = self.paid_queue.get(&claim) {
                return Err(AllocationConflict::PaidQueue { claim, owner });
            }
            self.paid_queue.insert(claim, owner);
        }
    ...
    fn resolve_with_funding_mode(
        ...
        let mut search = ProductionPortfolioSearch::for_claims(
            ...
            self,
            funding_mode,
            production_bounds::ProductionBounds::new(&self.producer_jobs, &producers).into(),
        );
        if !search.find(
            &mut producers,
...
impl<'a> ProductionPortfolioSearch<'a> {
    fn for_claims(
        capacity: &'a AllocationCapacity,
        claims: &'a ClaimState,
        funding_mode: JointFundingMode,
        ...
        let guarded_minimum_residual_scrap = claims
            .minimum_residual_scrap
        ...
        Self {
            ...
            minimum_residual_scrap: claims.effective_minimum_residual_scrap(),
            guarded_minimum_residual_scrap,
            voluntary_scrap_guard: claims.voluntary_scrap_guard,
            ...
            earliest_enqueue_dominates: current_bank_covers_all_claims(
                ...
                guarded_minimum_residual_scrap,
                &claims.forecast_scrap,
                &claims.deferrable_capital,
    ...
    fn optimistic_funding_fits(&self) -> bool {
        if self.earliest_enqueue_dominates || self.jobs.is_empty() {
            return true;
        }
        let Some(mut schedule) = optimistic_funding_schedule(self.jobs) else {
            return false;
        };
    ...
    fn is_frontier(&self, job_index: usize, remaining: &[bool]) -> bool {
        ...
        !self.jobs.iter().enumerate().any(|(index, other)| {
            remaining[index]
                && other.claim.fixed_assignment().is_some_and(|other_fixed| {
                    other_fixed.producer == fixed.producer
                        && (
                            other_fixed.enqueued_at,
                            other_fixed.starts_at,
                            other.funding_priority,
                            other.owner,
                            other.ordinal,
                        ) < (
                            fixed.enqueued_at,
...
fn assign_joint_funding(
    ...
    match mode {
        ...
        JointFundingMode::PreserveCompatiblePortfolio => {
            ...
            let before_retained = funding.clone();
            if funding.assign_preferred(&flexible[..fresh_start]).is_none() {
                funding = before_retained;
                funding.assign_deadline_compatible(&flexible[..fresh_start])?;
            }
...
pub(crate) fn voluntary_construction_admission_reserve(
    voluntary_guard: u32,
    minimum_residual_scrap: u32,
) -> u32 {
    voluntary_guard
...
mod tests {
    ...
    #[test]
    fn paid_queue_occurrences_are_exclusive_cost_free_and_transactional() {
        let mut basis = capacity(0, 0, vec![], vec![]);
        basis
            .paid_queue
            .insert((BuildingId(2), UnitKind::Kestrel), 2);
        let claim = PaidQueueClaim {
            producer: BuildingId(2),
            kind: UnitKind::Kestrel,
            occurrence: 0,
        };
        let first = ClaimOwner::Proposal(foundry(0, 0, vec![], ordinary_case()).key());
        let second = ClaimOwner::Proposal(standing(UnitKind::Warden, 0, ordinary_case()).key());
        let mut state = ClaimState::default();
        state
            .try_apply(
                &basis,
                first,
                &ClaimBundle::default().with_paid_queue(vec![claim]),
            )
            .unwrap();
        let checkpoint = state.clone();
        assert_eq!(
            state.current_scrap, 0,
            "paid inventory is not charged a second time"
        );
        assert_eq!(
            state
                .try_apply(
                ...
                .unwrap_err(),
            AllocationConflict::PaidQueue {
                claim,
                owner: first
            }
        );
        assert_eq!(state, checkpoint);
        let next = PaidQueueClaim {
            occurrence: 1,
            ..claim
        };
        state
            .try_apply(
                &basis,
                second,
                &ClaimBundle::default().with_paid_queue(vec![next]),
            )
            .unwrap();
        let checkpoint = state.clone();
        let absent = PaidQueueClaim {
            occurrence: 2,
            ..claim
        };
        assert_eq!(
            state
                .try_apply(
                    &basis,
                    second,
                    &ClaimBundle::default().with_paid_queue(vec![absent])
                )
                .unwrap_err(),
            AllocationConflict::UnknownPaidQueue(absent)
        );
        assert_eq!(state, checkpoint);
    }
    ...
    fn a_three_foundation_conflict_preserves_every_legal_pair() {
        ...
        let third = InvestmentProposal::fresh(
            ProposalKey::Economy(crate::utility::EconomicInvestmentKey::Build {
        ...
        let layouts =
            [
                IncompatibleLayoutSet::from_keys(vec![third.key(), first.key(), second.key()])
                    .unwrap(),
            ];
        ...
        assert!(result.decisions.iter().any(|decision| matches!(
            ...
            ProposalDisposition::Rejected(ProposalRejection::ConflictsWithSelected {
                conflict: AllocationConflict::IncompatibleLayout { keys },
                ..
            }) if keys.len() == 3
        )));
    }
    ...
    #[test]
    fn a_four_member_layout_certificate_preserves_all_proper_subsets() {
        let proposals = vec![
            ...
            InvestmentProposal::fresh(
                ...
                ordinary_case(),
                bundle(0, vec![], vec![], vec![], vec![], vec![]),
                "economy",
            ),
            InvestmentProposal::fresh(
                ProposalKey::Support(crate::utility::SupportKey {
                    patient: oxide_sim::ids::Target::Building(BuildingId(8)),
                    worker: UnitId(4),
                }),
                ordinary_case(),
                bundle(0, vec![], vec![], vec![], vec![], vec![]),
        ...
        let keys = proposals
            ...
            .map(InvestmentProposal::key)
            .collect::<Vec<_>>();
        let certificate = IncompatibleLayoutSet::from_keys(keys.clone()).unwrap();
    ...
    fn an_incompatible_foundry_and_defense_layout_is_rejected_without_reranking() {
        ...
        assert!(matches!(
            ...
            Some(AllocationConflict::IncompatibleLayout { keys })
                if *keys == incompatible.keys
        ));
    }
    ...
    #[test]
    fn retained_future_jobs_keep_matured_income_against_later_capital_deadlines() {
        let producer = BuildingId(7);
        let kind = UnitKind::Harvester;
        let cost = kind.stats().cost;
        let enqueued_at = 24;
        let ready_at = enqueued_at + u64::from(kind.stats().train_ticks) - 1;
        let capital = ImportedObligation {
            class: ObligationClass::PersistentPlan,
            accepted_at: 0,
            key: ObligationKey::SavedFoundry {
                anchor: TilePos::new(42, 22),
        ...
        let production = ImportedObligation {
            class: ObligationClass::PersistentPlan,
            accepted_at: 5,
            key: ObligationKey::ConnectedOffense {
                objective: BuildingId(90),
        ...
        for (now, bank) in [(0, 0), (12, cost)] {
            let mut income = vec![ForecastAvailability {
                available_at: 2000,
                amount: 300,
            }];
            if bank == 0 {
            ...
            .unwrap();
            let basis = timed_capacity(bank, now, 2000, 1, income, vec![lane]);
            let result = allocate::<()>(
                &basis,
                vec![capital.clone(), production.clone()],
                vec![],
                AllocationPersonality::default(),
            )
            .expect("arrival of promised income cannot invalidate retained deadlines");
            let job = &result.producer_schedule[0];
            assert_eq!(
                (job.enqueued_at, job.starts_at, job.ready_at),
                (enqueued_at, enqueued_at, ready_at)
            );
            assert_eq!(job.current_scrap, bank);
            assert_eq!(job.forecast_scrap, cost - bank);
            assert_eq!(result.capital_assignments[0].forecast_scrap, 300);
        }
    }
    ...
    fn fixed_producer_chronology_is_independent_of_funding_priority() {
        ...
        let basis = timed_capacity(
            ...
            vec![
                timed_producer_fixture(
                    ...
                    0,
                    vec![0; QUEUE_CAP],
                    vec![UnitKind::Lancer, UnitKind::Bombard],
                ),
                timed_producer_fixture(
                    BuildingId(54),
                    0,
                    ...
                    0,
                    vec![0; QUEUE_CAP],
                    vec![UnitKind::Gnat, UnitKind::Darter],
                ),
            ],
        );
        let mut jobs = vec![
            ProducerJobClaim::fixed(BuildingId(54), UnitKind::Gnat, 468, 468, 567, 2_400),
            ProducerJobClaim::fixed(BuildingId(7), UnitKind::Bombard, 1_068, 1_068, 1_367, 2_400),
        ];
        jobs.extend([816, 1_116, 1_416].map(|release| {
            ProducerJobClaim::flexible(UnitKind::Darter, release, 2_400, vec![BuildingId(54)])
        }));
        let mut jobs: Vec<_> = jobs
            ...
            .collect();
        jobs.push(OwnedProducerJob {
            claim: ProducerJobClaim::fixed(BuildingId(7), UnitKind::Lancer, 336, 336, 535, 2_400),
            owner: earlier_owner,
            ordinal: 0,
            funding_priority: FundingPriority::fresh_proposal(earlier_owner, 1),
        });
        let claims = ClaimState {
            producer_jobs: jobs,
            minimum_residual_scrap: 79,
            ..ClaimState::default()
        };
        let resolved = claims
            .resolve(&basis)
            .expect("the forecast funds all six jobs");
        assert_eq!(resolved.producer_schedule.len(), 6);
        assert_eq!(resolved.producer_schedule[0].kind, UnitKind::Lancer);
        assert_eq!(resolved.producer_schedule[0].enqueued_at, 336);
        assert!(resolved.search_states <= 7);
    }
