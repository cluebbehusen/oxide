...
use crate::executive::{Intent, building_strength, full_ground_strength, ground_strength};
use crate::intelligence::{ContactEvidence, UnitContact};
use crate::navigation::public_fields::{PublicGroundDistances, PublicRoutes};
...
fn foundry_drip_per_minute() -> u64 {
    u64::from(oxide_sim::TICKS_PER_SECOND).saturating_mul(60)
...
pub(super) struct ExpansionAssessmentContext<'a> {
    ...
    pub(super) uncleared_hostile_starts: &'a [crate::StartingFoundry],
...
impl KnownGroundThreat {
    ...
    fn from_current(unit: &crate::observation::UnitObs, now: u64) -> Option<Self> {
        let strength = GroundStrength(crate::executive::unit_strength(unit));
...
mod tests {
    ...
    fn building(
        ...
        kind: BuildingKind,
        anchor: TilePos,
    ) -> crate::observation::BuildingObs {
        crate::test_support::building(id, oxide_sim::ids::PlayerId(player), kind, anchor)
    }
    ...
    fn unit(
        ...
        tile: TilePos,
        hp: u32,
    ) -> crate::observation::UnitObs {
        crate::observation::UnitObs {
            hp,
            ..crate::test_support::unit(id, oxide_sim::ids::PlayerId(player), kind, tile)
        }
    }
    ...
    fn assessment_context<'a>(
        ...
        starts: &'a [crate::StartingFoundry],
    ...
    fn security_rejection_falls_through_to_the_next_economic_plan() {
        ...
        let enemy_start = crate::StartingFoundry {
    ...
    fn candidate_local_security_cost_can_reverse_the_economic_order() {
        let obs = observed_map(20, 8, TilePos::new(2, 3));
        let starts = [crate::StartingFoundry {
            player: oxide_sim::ids::PlayerId(1),
            anchor: TilePos::new(16, 3),
        }];
        let public_map = briefing(20, 8, [], starts.to_vec());
    ...
    fn assessment_excludes_reserved_core_but_counts_queued_and_same_think_core() {
        ...
        let same_think = [Intent::TrainAt {
            building: oxide_sim::ids::BuildingId(0),
            kind: UnitKind::Sentinel,
        }];
        let public_map = briefing(12, 8, [], Vec::new());
