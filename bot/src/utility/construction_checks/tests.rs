...
fn trailing_stealthy_build_cannot_hide_an_earlier_producer_seal() {
    ...
    let map = briefing();
    let mut builds: Vec<_> = oxide_sim::geometry::rect_adjacent_tiles(
        LEFT_HOME,
...
fn resource_access_evidence_is_shared_and_invalidates_on_effective_inputs() {
    ...
    let (baseline, cold) =
        crate::navigation::work::measure(|| scrap_assets(&policy, &ground, &[LEFT_HOME]));
