...
use crate::intelligence::{BuildingContact, UnitContact};
...
impl HarvestDangerLayout {
    fn from_observation(
        ...
        rectangles.extend(
            obs.blips.iter().map(|&tile| {
                DangerRect::around(tile, oxide_sim::stats::HARVEST_RADAR_DANGER_RADIUS)
            }),
        );
            ...
            }))
        {
            if let Some(radius) = kind
                ...
                .map(|weapon| {
                    (weapon.range + oxide_sim::stats::HARVEST_MOBILE_DANGER_MARGIN)
                        .ceil()
                        .to_num::<i32>()
                })
                .max()
        ...
        for building in &obs.enemy_buildings {
            ...
            if let Some(radius) = stats
                ...
                .filter(|weapon| weapon.targets.covers(Domain::Ground))
                .map(|weapon| {
                    (weapon.range + oxide_sim::stats::HARVEST_STATIC_DANGER_MARGIN)
                        .ceil()
                        .to_num::<i32>()
                })
                .max()
...
fn location_has_known_danger(
    ...
    if obs.blips.iter().any(|contact| {
        contact.chebyshev(node) <= oxide_sim::stats::HARVEST_RADAR_DANGER_RADIUS + additional_margin
    }) {
        return true;
    }
...
mod tests {
    ...
    fn projection_matches_the_direct_predicate_for_every_danger_evidence_shape() {
        ...
        obs.enemy_buildings.push(BuildingObs {
            seen: false,
            ..crate::test_support::building(u32::MAX, PlayerId(1), BuildingKind::Turret, anchor)
        });
