        ...
        let (units, jobs) = match self.observer {
            ...
            ReconObserver::Queued {
                ...
                return ClaimBundle::default().with_paid_queue(vec![
                    crate::allocation::PaidQueueClaim {
                        producer,
                        kind,
                        occurrence,
                    },
...
impl ReconPhase {
    pub(crate) fn trace(self) -> crate::trace::ReconPhaseTrace {
        use crate::trace::ReconPhaseTrace;
        match self {
...
pub(crate) struct ReconAssignment {
    ...
    pub(crate) paid_claim: Option<crate::allocation::PaidQueueClaim>,
    pub(crate) unpaid: bool,
    pub(crate) funding: Option<crate::allocation::ScheduledProducerJob>,
}
...
pub(crate) struct Reconnaissance {
    ...
    capability_demand: Vec<crate::standing_force::CapabilityDemand>,
    queue_counts: BTreeMap<(BuildingId, UnitKind), usize>,
    previous_units: Vec<UnitId>,
    pub(crate) released: Vec<(ReconQuestionKey, crate::trace::ReconReleaseReason)>,
...
impl Reconnaissance {
    ...
    fn record_loss(&mut self, key: ReconQuestionKey, now: Tick) {
        ...
        self.released
            .push((key, crate::trace::ReconReleaseReason::ObserverLost));
    }
    pub(crate) fn release_unpaid(
        &mut self,
        key: ReconQuestionKey,
        now: Tick,
        reason: crate::trace::ReconReleaseReason,
    ) {
    ...
    pub(crate) fn paid_claims(&self) -> Vec<crate::allocation::PaidQueueClaim> {
        self.assignments
            .values()
...
fn newborn_at_factory(
    ...
    distance <= 1
        ...
        && context
            ...
            .all(|building| {
                unit.tile
                    .chebyshev(crate::navigation::commands::air_production_spawn_tile(
                        building,
...
impl UtilityPolicy {
        ...
        for frame in map.extractor_frames() {
            ...
            let actionable = obs.my_buildings.iter().any(|building| {
                building.kind == BuildingKind::Foundry
                    && building.built
                    && building.anchor.chebyshev(*frame)
        ...
        if harvesters > 0 {
            ...
            for (source, prior_amount) in sources {
                ...
                let useful = obs
                    ...
                    .any(|base| {
                        let local = source.chebyshev(base.anchor)
                            <= oxide_sim::stats::EXTRACTOR_SUPPORT_RADIUS;
                        let known_work = obs
                            .known_scrap
                            .iter()
    ...
    pub(crate) fn observe_reconnaissance(
        &mut self,
        context: EconomicInvestmentContext<'_>,
        home: TilePos,
    ) -> Vec<Intent> {
            ...
            if work.phase == ReconPhase::Preparation {
                    ...
                    || (work.unpaid && obs.tick >= work.proposal.funding_deadline())
                {
                    let reason = if answered {
                        crate::trace::ReconReleaseReason::Answered
                    } else if !useful {
                        crate::trace::ReconReleaseReason::NoLongerUseful
                    } else {
                        crate::trace::ReconReleaseReason::DeadlineExpired
                    };
                    ...
                    })
                {
                    self.state.reconnaissance.released.push((
                        key,
                        crate::trace::ReconReleaseReason::ProducerOrRouteUnavailable,
                    ));
                    self.state
                        .reconnaissance
                        .reconsider_after_quiet(key, obs.tick);
                ...
                if work.phase == ReconPhase::Recall {
                    if unit.tile.chebyshev(return_goal) <= 1 {
                        ...
                        self.state
                            .reconnaissance
                            .released
                            .push((key, crate::trace::ReconReleaseReason::SafeReturn));
                        continue;
                    }
    ...
    pub(crate) fn prepare_reconnaissance(
        &mut self,
        context: EconomicInvestmentContext<'_>,
        tuning: DifficultyTuning,
        minimum_core: u32,
        allow_paid: bool,
        paid_unavailable: &[crate::allocation::PaidQueueClaim],
    ) -> Vec<ReconProposal> {
        ...
        for question in questions {
                    ...
                    })
            {
                ...
                let coverage = live
                    ...
                    .or_else(|| {
                        let kind = oxide_sim::stats::Role::Scout.unit_for(obs.faction);
                        for lane in context.resources.producers() {
                            ...
                            for (occurrence, (_, ready_at)) in lane
                                ...
                                .enumerate()
                            {
                                if !operation.paid.contains(&crate::allocation::PaidQueueClaim {
            ...
            for lane in context.resources.producers() {
                ...
                for (occurrence, (_, ready_at)) in lane
                    ...
                    .enumerate()
                {
                    let claim = crate::allocation::PaidQueueClaim {
                        producer: lane.producer,
                        kind,
                        occurrence,
                    };
            ...
            if let Some((arrival_at, producer, ready_at, origin, goal)) = producers.first().copied()
            {
                    ...
                    && routes.air.command_path_avoids_blocked(home, goal)
                {
                    ...
                    self.state.reconnaissance.capability_demand.push(
                        crate::standing_force::CapabilityDemand {
                            kind,
                            service: crate::allocation::StandingForceServiceKey::Point(
                                question.key.tile(),
                            ),
                            reason: crate::standing_force::StandingForceReason::Reconnaissance,
                            case: ProposalCase {
    ...
    pub(crate) fn bind_reconnaissance_funding(
        ...
        if job.enqueued_at == obs.tick {
            ...
            work.paid_claim = Some(crate::allocation::PaidQueueClaim {
                producer,
                kind,
                occurrence,
            });
    ...
    pub(crate) fn bind_reconnaissance_queue_order(
        &mut self,
        jobs: &[crate::allocation::ScheduledProducerJob],
        obs: &Observation,
    ) {
...
mod tests {
    ...
    fn recently_answered_objective_is_not_an_immediate_new_purchase() {
        ...
        building.seen = true;
        obs.enemy_buildings.push(building);
        let mut intelligence = crate::intelligence::StrategicIntelligence::new();
    ...
    fn partial_funding_owns_one_frozen_purchase_without_emitting_forecast_commands() {
        ...
        let obligation = imported_obligation(
            crate::allocation::ObligationClass::PersistentPlan,
            proposal.observed_at,
            crate::allocation::ObligationKey::Reconnaissance(key),
