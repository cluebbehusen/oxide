...
use super::*;
use crate::navigation::egress::GroundEgressCache;
#[cfg(test)]
...
impl<'a> PlacementGeometry<'a> {
    ...
    pub(super) fn valid(
        ...
        #[cfg(test)]
        crate::navigation::work::record(|work| work.placement_checks += 1);
...
impl UtilityPolicy {
    ...
    fn ground_flood(
        ...
        crate::navigation::flood::reaches_any(
            obs.map_width,
            obs.map_height,
            [home],
            enter,
    ...
    fn planned_ground_open(obs: &Observation, tile: TilePos, planned: &[PlannedFootprint]) -> bool {
        crate::navigation::commands::ground_open(QueryPurpose::NavigationTest, obs, tile)
    ...
    fn tile_open(&self, obs: &Observation, t: TilePos) -> bool {
        ...
        let covered = |b: &crate::observation::BuildingObs| {
...
mod tests {
    ...
    fn indexed_placement_matches_scalar_geometry_across_known_obstacles() {
        ...
        obs.enemy_units.push(UnitObs {
            hp: 60,
            ..crate::test_support::unit(90, PlayerId(1), UnitKind::Sentinel, TilePos::new(10, 10))
        });
    ...
    fn construction_placement_respects_a_walking_founders_promised_footprint() {
        ...
        ));
        {
            ...
            obs.my_units.push(UnitObs {
                ...
                ..crate::test_support::unit(70, obs.me, UnitKind::Harvester, TilePos::new(12, 10))
            });
        }
        assert!(!placement_valid(
            &policy,
    ...
    fn fresh_foundations_do_not_displace_an_active_builder_from_its_work_tile() {
        ...
        obs.my_buildings[1].built = false;
        {
            ...
            obs.my_units.push(UnitObs {
                ...
                ..crate::test_support::unit(70, obs.me, UnitKind::Harvester, work)
            });
        }
    ...
    fn a_parked_hostile_airframe_closes_a_placement_tile_until_it_lifts_off() {
        ...
        obs.enemy_units.push(UnitObs {
            grounded: true,
            ..crate::test_support::unit(90, PlayerId(1), UnitKind::Condor, tile)
        });
        assert!(
            !policy.placement_tile_open_except(
                &obs,
    ...
    fn dense_placement_may_replace_the_first_spawn_when_an_egress_remains() {
        ...
        let first_spawn = oxide_sim::geometry::rect_adjacent_tiles(
            foundry.anchor,
    ...
    fn active_scuttle_charge_foundation_is_nonblocking_in_cold_and_cached_egress() {
        let obs = observation();
        let foundry = &obs.my_buildings[0];
        let mine_anchor = oxide_sim::geometry::rect_adjacent_tiles(
            foundry.anchor,
            foundry.kind.base_stats().size,
        )
        .find(|tile| UtilityPolicy::planned_ground_open(&obs, *tile, &[]))
        ...
        with_mine.my_units.push(UnitObs {
            ...
            ..crate::test_support::unit(100, PlayerId(0), UnitKind::Harvester, TilePos::new(2, 2))
        });
        assert!(
    ...
    fn an_already_sealed_producer_does_not_paralyze_other_placement() {
        ...
        for anchor in
            oxide_sim::geometry::rect_adjacent_tiles(sealed.anchor, sealed.kind.base_stats().size)
        {
            add_building(&mut obs, BuildingKind::Barricade, anchor);
        }
        let usable_ring: Vec<_> =
            oxide_sim::geometry::rect_adjacent_tiles(usable.anchor, usable.kind.base_stats().size)
                .collect();
    ...
    fn dense_base_grid_matches_the_routing_projection() {
        ...
        obs.my_units.push(UnitObs {
            ...
            ..crate::test_support::unit(100, PlayerId(0), UnitKind::Harvester, TilePos::new(1, 2))
        });
        let open =
            GroundEgressCache::ground_egress_base_open(&obs, FoundationCancellations::default());
    ...
    fn every_ground_producer_keeps_egress_not_only_the_home_foundry() {
        ...
        let ring: Vec<_> = oxide_sim::geometry::rect_adjacent_tiles(
            fabricator.anchor,
            fabricator.kind.base_stats().size,
        )
        .collect();
        for &anchor in &ring[..ring.len() - 1] {
            add_building(&mut obs, BuildingKind::Reclaimer, anchor);
    ...
    fn egress_cache_layout_tracks_every_routing_input() {
        ...
        with_building
            .enemy_buildings
            .push(crate::test_support::building(
                100,
                PlayerId(1),
                BuildingKind::Turret,
                TilePos::new(1, 1),
        ...
        with_founding.my_units.push(UnitObs {
            ...
            ..crate::test_support::unit(100, PlayerId(0), UnitKind::Harvester, TilePos::new(1, 1))
        });
