...
use super::construction_checks::KnowledgeDomain as DefenseDomain;
use super::construction_checks::*;

use super::*;
use crate::intelligence::ContactEvidence;
use crate::navigation::paths::{EndpointRoutes, path_cost};
...
use oxide_sim::stats::{CHARGE_TRIGGER_RADIUS, SAPPER_CONTACT_RANGE, WeaponStats};
...
pub(super) struct DefenseGrounding<'a> {
    construction: ConstructionGrounding<'a>,
...
pub(super) struct DefenseThinkCacheStats {
    ...
    pub(super) coverage_batches: usize,
    pub(super) coverage_scores: usize,
}
...
impl<'a> DefenseGrounding<'a> {
    ...
    fn new_inner(
        ...
        let construction =
            ConstructionGrounding::new(query_purpose, policy, obs, briefing, orientation);
        let assets = valued_assets(obs, construction.resources.clone());
        Self {
            construction,
            assets,
        }
...
impl<'a> DefenseThinkContext<'a> {
    ...
    pub(super) fn safe_implicit_builder(
        ...
        cached_safe_implicit_builder(
            ...
            BuilderSafetyContext {
                obs: self.obs,
                routes: &self.grounding.construction.build_routes,
                danger: &self.danger,
    ...
    fn array_upgrade_value(&self, building: &BuildingObs, horizon: u64, offline: u64) -> u64 {
        let covers = |array: &BuildingObs, target: TilePos, tier: u8| {
            let radius = if tier == 0 {
                oxide_sim::stats::CHARGE_BASE_ARRAY_DETECT_RADIUS
            } else {
                oxide_sim::stats::CHARGE_ARRAY_DETECT_RADIUS
            };
        ...
        let radius = oxide_sim::stats::CHARGE_ARRAY_DETECT_RADIUS;
    ...
    pub(super) fn reinforcement_travel_cost(
        ...
        let goals = if movement_domain == DefenseDomain::Air {
            ...
            building_doorsteps(&self.grounding.construction.ground, anchor, footprint.size)
        };
        let candidate = (movement_domain == DefenseDomain::Ground).then_some(footprint);
        let board = routing_cache::board(&self.grounding.construction.ground, movement_domain);
        let cost = board
            .cache
            .borrow_mut()
            .costs
            .between_sets(
                self.grounding.construction.ground.query_purpose,
                board.grid,
    ...
    fn ensure_projection(&mut self, domain: DefenseDomain) {
        ...
        if slot.is_none() {
            if let crate::planning::Progress::Ready(projection) = prepare_strategic_lane_projection(
                self.obs,
                self.unit_contacts,
                self.building_contacts,
                &self.grounding,
    ...
    pub(super) fn cache_stats(&self) -> DefenseThinkCacheStats {
        DefenseThinkCacheStats {
            ...
            future_producer_baseline_builds: self
                .grounding
                .construction
                .future_ground_producers
                .len(),
            builder_safety_builds: self.evaluation.construction.stats.builder_safety_builds,
            builder_safety_hits: self.evaluation.construction.stats.builder_safety_hits,
            builder_travel_builds: self.evaluation.construction.stats.builder_travel_builds,
            builder_travel_hits: self.evaluation.construction.stats.builder_travel_hits,
            future_producer_egress_builds: self
                .evaluation
                .construction
                .stats
                .future_producer_egress_builds,
            future_producer_egress_hits: self
                .evaluation
                .construction
                .stats
                .future_producer_egress_hits,
            resource_access_builds: self.evaluation.construction.stats.resource_access_builds,
            resource_access_hits: self.evaluation.construction.stats.resource_access_hits,
...
impl<'a> ResourceAccessGuard<'a> {
    ...
    pub(super) fn survives(&self, kind: BuildingKind, anchor: TilePos) -> bool {
        scrap_access_survives(
            &self.ground,
            self.assets.iter().filter_map(|asset| asset.access.as_ref()),
            PlacementFootprint {
                anchor,
                size: kind.base_stats().size,
                blocks_ground: !kind.is_stealthy(),
    ...
    fn candidate_seeds(&self) -> Vec<TilePos> {
        match self {
            Self::Building { anchor, size } => {
                sorted_tiles(oxide_sim::geometry::rect_adjacent_tiles(*anchor, *size))
            }
    ...
    fn approach_tiles(self, ground: &GroundKnowledge<'_>, domain: DefenseDomain) -> Vec<TilePos> {
        match (self.size, domain) {
            (Some(size), DefenseDomain::Ground) => sorted_tiles(
                oxide_sim::geometry::rect_adjacent_tiles(self.anchor, size)
                    .filter(|tile| ground.open(*tile, None, domain)),
            ),
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum DefenseOpportunityEvidence {
...
impl UtilityPolicy {
    pub(super) fn current_emergency_defense_required(
        ...
        let ground = GroundKnowledge::new(
            crate::query_work::QueryPurpose::DefenseSitePlacement,
            &unclaimed,
            briefing,
            &public_starts,
        )
    ...
    pub(super) fn current_emergency_defense_claim_is_useful(
        ...
        if !scrap_access_survives(
            &ground,
            assets.iter().filter_map(|asset| asset.access.as_ref()),
            placement,
            None,
        ) {
            return false;
...
fn prepare_strategic_lane_projection<'a>(
    ...
    grounding: &DefenseGrounding<'a>,
    domain: DefenseDomain,
) -> crate::planning::Progress<Option<StrategicLaneProjection<'a>>> {
    if grounding.assets.is_empty() {
        return crate::planning::Progress::Ready(None);
    }
    for (tier, origins) in threat_origin_tiers(
        obs,
        unit_contacts,
        building_contacts,
        &grounding.construction.public_starts,
        domain,
    )
    .into_iter()
    .enumerate()
    ...
    {
        let approaches = match fronts::approaches(
            &grounding.construction.ground,
            &origins,
            &grounding.assets,
            domain,
        ) {
        ...
        return crate::planning::Progress::Ready(Some(StrategicLaneProjection {
            origins,
            approaches,
            evidence,
    ...
    crate::planning::Progress::Ready(None)
}
...
fn emergency_lane_projection<'a>(
    ...
    let approaches = emergency_approaches(
        &grounding.construction.ground,
        &origins,
        &grounding.assets,
        domain,
...
fn strategic_defense_quote_from_projection(
    ...
    let profile = DefenseProfile::for_kind(kind)?;
    let ground = &grounding.construction.ground;
    let assets = &grounding.assets;
    let origins = &projection.origins;
    let approaches = &projection.approaches;

    let coverage = (kind != BuildingKind::Barricade).then(|| {
        #[cfg(test)]
        {
            cache.stats.coverage_batches += 1;
        }
        coverage::Batch::new(
            &CoverageContext {
                obs,
    ...
    let mut evaluate = |anchor| {
        ...
        let candidate = (|| {
            if !grounding.construction.placement.valid(policy, kind, anchor) {
                return None;
            }
            let placement = profile.footprint(anchor);
            ...
            let (builder, builder_travel) = ordered_builders.into_iter().find_map(|builder| {
                let safe = cached_safe_implicit_builder(
                    ...
                    BuilderSafetyContext {
                        obs,
                        routes: &grounding.construction.build_routes,
                        danger,
            ...
            if !cached_resource_access_survives(
                &grounding.construction,
                &mut cache.construction,
                placement,
                resource_detour_limit,
            ) {
                return None;
            }
...
fn candidate_threat_summary<'a>(
    ...
    profile: DefenseProfile,
    candidate: TilePos,
    approaches: impl IntoIterator<Item = &'a Approach>,
...
fn defended_assets(
    ...
    valued_assets(obs, ground.resource_assets(policy))
}

fn valued_assets(obs: &Observation, resources: Vec<ResourceRegion>) -> Vec<DefendedAsset> {
    ...
    assets.extend(resources.into_iter().map(|region| {
        DefendedAsset {
            value: region
                .scrap
                .div_ceil(oxide_sim::stats::SCRAP_NODE_AMOUNT)
                .min(8),
            shape: AssetShape::Scrap {
                tiles: region.tiles,
                work_tiles: region.work_tiles,
            },
            access: Some(region.access),
        }
    }));
...
fn approach_path(
    ...
    field: Option<
        &std::cell::OnceCell<
            crate::planning::Progress<std::sync::Arc<crate::navigation::approaches::ApproachField>>,
        >,
    >,
...
) -> Result<Option<(TilePos, Vec<TilePos>)>, crate::navigation::paths::PendingRoute> {
...
fn tiles_reachable_within_steps(
    ...
    crate::navigation::flood::within_steps(
...
fn defense_candidate_tiles(
    ...
    let mut candidate_tiles = crate::navigation::areas::square_neighborhoods(
        obs.map_width,
        obs.map_height,
        seeds,
...
fn reference_score_coverage(
    ...
    for (asset_index, asset) in assets.iter().enumerate() {
        ...
        let mut novel_tiles = BTreeSet::new();
        ...
        if protects {
            coverage.novel_span = coverage.novel_span.saturating_add(
                asset
                    .value
                    .saturating_mul(novel_tiles.len().min(INTERCEPTION_DEPTH) as u32),
            );
...
mod tests {
    ...
    fn defensive_refit_values_local_protection_without_searching_army_routes() {
        ...
        let ((benefit, evidence), work) =
            crate::navigation::work::measure(|| context.upgrade_quote(&turret, 6_000));
    ...
    fn site_bounds_preserve_exact_quotes_across_roles_and_obstacles() {
        for shape in 0..3 {
            let scenario = scenario_with(|tile| {
                if shape == 1 && tile.x == 20 && !matches!(tile.y, 5 | 18)
                    || shape == 2 && tile.y == 10 && (12..28).contains(&tile.x)
                {
                    '#'
                } else {
                    '.'
                ...
                for kind in [
                    ...
                    let projection =
                        strategic_lane_projection(&obs, &[], &[], &grounding, profile.domain)
                            .expect("public enemy starts must produce approach routes");
                    ...
                    for y in (0..obs.map_height).step_by(4) {
                        for x in (0..obs.map_width).step_by(4) {
                            ...
                            let exact = Candidate {
                                anchor,
                                builder_travel: 0,
                                coverage: score_coverage(&context, profile, anchor),
                                threat_distance: projection
                                    .origins
                                    .iter()
                            ...
                            let bound = bounds.candidate(
                                &CoverageContext {
                                    approaches: &projection.approaches,
                                    ..context
                                },
                                profile,
                                anchor,
                            ...
                            assert!(
                                bound.key(profile) >= exact.key(profile),
                                "inadmissible bound: shape {shape}, defended {defended}, kind {kind:?}, anchor {anchor:?}"
                            );
                        }
                    }
                    assert_eq!(
                        run(DefenseSiteSearch::Best),
                        run(DefenseSiteSearch::Exhaustive),
                        "shape {shape}, defended {defended}, kind {kind:?}"
                    );
                }
            }
        }
    ...
    fn builder_travel_uses_the_authoritative_rotated_doorstep() {
        ...
        assert_eq!(
            grounding
                .construction
                .builder_travel_cost(&obs.my_units[0], placement, None),
            Some(54),
        ...
        assert_eq!(
            grounding
                .construction
                .builder_travel_cost(&obs.my_units[1], placement, None),
            Some(50),
    ...
    fn barricade_quotes_use_shared_cost_queries_across_decisions() {
        ...
        let (first, cold_navigation) = crate::navigation::work::measure(quote);
        ...
        let (second, warm_navigation) = crate::navigation::work::measure(quote);
    ...
    #[test]
    fn candidate_enumeration_does_not_run_route_or_egress_proofs() {
        let obs = observation(PlayerId(0), LEFT_HOME);
        let map = briefing();
        let policy = UtilityPolicy::new();
        let starts = policy.uncleared_hostile_starts(&map, obs.me);
        ...
        let (candidates, work) = crate::navigation::work::measure(|| {
            defense_candidate_tiles(
                &policy,
                &obs,
                &assets,
    ...
    fn one_think_context_reuses_exact_projection_and_route_answers() {
        ...
        assert!(stats.coverage_batches > 0);
        assert!(stats.coverage_scores > stats.coverage_batches);
    ...
    fn local_support_uses_one_bounded_flood_for_all_assets() {
        ...
        let (supported, work) = crate::navigation::work::measure(|| {
    ...
    fn tile_resolution_dead_zone_projection_matches_the_sim_retreat_goal() {
        ...
        state.tick(&[PlayerCommand {
            ...
            command: Command::Attack {
                units: vec![avalanche],
                target: oxide_sim::AttackTarget::Building(foundry),
                queue: false,
            },
        }]);
        let sim_goal = state
    ...
    fn voluntary_fronts_share_clumped_routes_but_keep_directions_and_weapon_roles() {
        ...
        let crate::planning::Progress::Ready(selected) =
    ...
    fn a_rectangular_footprint_on_one_diagonal_companion_keeps_resource_access() {
        ...
        assert!(
            !scrap_access_survives(&ground, resource.access.iter(), barricade, Some(detour - 1),),
    ...
    fn explored_depletion_overrides_authored_scrap_while_dark_ground_keeps_the_prior() {
        ...
        assert_eq!(
            ...
            Some(&oxide_sim::stats::RICH_SCRAP_NODE_AMOUNT)
        );
    ...
    fn emergency_range_pruning_preserves_artillery_and_exact_nearby_routes() {
        for scenario in [
            ...
            let (result, work) = crate::navigation::work::measure(|| {
