...
fn unit(id: u32, kind: UnitKind) -> UnitObs {
    crate::test_support::unit(
        ...
        PlayerId(0),
        kind,
        TilePos::new(3 + i32::try_from(id % 10).unwrap(), 6),
    )
}
...
fn core_status_counts_only_unreserved_line_hulls_and_exact_pipeline_work() {
    ...
    let expected = crate::executive::unit_strength(&wounded)
        .saturating_add(crate::executive::unit_strength(&ordinary))
    ...
    assert_eq!(
        ...
        expected.saturating_add(crate::executive::unit_strength(&reserved))
    );
}
...
fn every_line_hull_uses_the_same_live_and_queued_strength_currency() {
    for kind in [UnitKind::Sentinel, UnitKind::Warden, UnitKind::Breaker] {
        ...
        assert_eq!(
            ...
            crate::executive::unit_strength(&unit(10, kind))
        );
    }
    for kind in [
        UnitKind::Scuttler,
