...
use crate::trace::{RepairProgramTrace, SupportLifecycleReason, SupportLifecycleTrace};
use chassis::Tick;
...
impl RepairAssignment {
    pub(crate) fn trace(&self) -> RepairProgramTrace {
        RepairProgramTrace {
...
#[derive(Clone)]
pub(crate) struct SupportWorkSnapshot {
    patients: Vec<Patient>,
    danger: std::sync::Arc<danger::HarvestDangerProjection>,
    pub(crate) protection: Vec<super::support_deployment::ProtectionRequest>,
}

impl SupportWorkSnapshot {
    pub(crate) fn unit_work(&self) -> Vec<crate::standing_force::RepairWork> {
        self.patients
            .iter()
            .filter_map(|patient| {
                ...
                Some(crate::standing_force::RepairWork {
                    unit,
                    tile: patient.tile,
...
impl Patient {
    fn covered_by_bay(self, anchor: TilePos) -> bool {
        ...
        chassis::fx::Fx::from_num(dx * dx + dy * dy)
            <= oxide_sim::stats::REPAIR_BAY_RADIUS * oxide_sim::stats::REPAIR_BAY_RADIUS
    }
    fn reserve(self, cadence: Tick) -> u32 {
        ...
        let debit =
            (hp * u128::from(self.basis) * u128::from(oxide_sim::stats::REPAIR_COST_PERMILLE))
...
impl UtilityPolicy {
    pub(crate) fn observe_support_work(&mut self, snapshot: &SupportWorkSnapshot, now: Tick) {
    ...
    pub(crate) fn discretionary_support_work(
        &self,
    ...
    pub(crate) fn support_work_snapshot(
        &self,
    ...
    pub(crate) fn prepared_repair_bays(
        &self,
        context: EconomicInvestmentContext<'_>,
        snapshot: &SupportWorkSnapshot,
        ...
        let funded_repairers: Vec<_> = self
            ...
            .collect();
        let support_context =
            crate::standing_force::StandingForceContext::new(context.unavailable, &[])
                .with_ground_routing(
                    crate::standing_force::StandingGroundTarget::point(TilePos::new(0, 0)),
        ...
        let mut service_routes = crate::navigation::service::ServiceRoutes::new(
        ...
        let unit_remaining: std::collections::BTreeMap<_, _> =
            crate::standing_force::remaining_repair_work(
                obs,
                support_context,
                context.resources,
                &mut service_routes,
        ...
        let mut geometry = super::construction_checks::ConstructionChecks::new(
            crate::query_work::QueryPurpose::SupportRouting,
            self,
            obs,
            context.briefing,
            ...
            let benefit = patients
                ...
                .map(|patient| {
                    let prior = existing
                        ...
                        .map(|(_, delay)| {
                            1_800_u64.saturating_sub(*delay) / oxide_sim::stats::REPAIR_BAY_PERIOD
                                * u64::from(oxide_sim::stats::REPAIR_BAY_STEP)
                        })
                        .sum::<u64>();
                    let assigned = self
                    ...
                    let repair_price = remaining.min(service)
                        ...
                        * oxide_sim::stats::REPAIR_COST_PERMILLE;
    ...
    pub(crate) fn renew_prepared_repairs(
        &mut self,
        context: EconomicInvestmentContext<'_>,
        snapshot: &SupportWorkSnapshot,
        available: u32,
    ...
    fn repair_patients(
        ...
        for building in &obs.my_buildings {
            ...
            let (ticks, basis) = if building.kind == BuildingKind::Foundry {
                (
                    oxide_sim::stats::FOUNDRY_REPAIR_TICKS,
    ...
    pub(crate) fn prepared_repair_assignments(
        &self,
        context: EconomicInvestmentContext<'_>,
        snapshot: &SupportWorkSnapshot,
    ) -> Vec<RepairAssignment> {
...
mod tests {
    ...
    fn context<'a>(
        ...
        profile: &'a crate::profile::ResolvedProfile,
    ...
    fn bay_admission_values_local_damage_and_delayed_overlapping_service() {
        ...
        covered.my_buildings.push(BuildingObs {
            built: false,
            ..crate::test_support::building(100, obs.me, BuildingKind::RepairBay, anchor)
        });
            ...
            for dx in -8..=8 {
                for dy in -8..=8 {
                    ...
                    let expected = on_bay.dist_sq(target.closest_point_to(on_bay))
                        <= oxide_sim::stats::REPAIR_BAY_RADIUS
                            * oxide_sim::stats::REPAIR_BAY_RADIUS;
                    assert_eq!(
            ...
            for cadence in [1, 6, 12, 24] {
                for phase in 0..stats.train_ticks.saturating_mul(2) {
                    let actual = (phase..phase + cadence)
                        .map(|tick| oxide_sim::stats::unit_repair_debit(kind, tick))
                        .sum::<u32>();
                    assert!(
