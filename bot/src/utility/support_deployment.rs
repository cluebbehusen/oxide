...
pub(crate) struct SupportDeployments {
    ...
    pub(crate) released: Vec<(SupportDeployment, crate::trace::DeploymentReleaseReason)>,
            ...
            .map(|building| {
                (
                    ...
                    building
                        ...
                        .construction
                        .map_or(oxide_sim::stats::FOUNDRY_REPAIR_PRICE, |stats| stats.cost),
                    true,
                )
            }),
    }
...
impl UtilityPolicy {
    pub(crate) fn discretionary_protection_work(
        &self,
        now: Tick,
    ...
    pub(crate) fn support_reservations(&self) -> Vec<UnitId> {
    ...
    pub(crate) fn observe_support_deployments(
        &mut self,
        context: EconomicInvestmentContext<'_>,
        ...
        for mut deployment in old {
            let Some(unit) = obs
                ...
                self.state.support_deployments.released.push((
                    deployment,
                    crate::trace::DeploymentReleaseReason::UnitUnavailable,
                ));
                continue;
            };
                ...
                || obs.my_queued_units.contains(&unit.id)
            {
                let reason = if target.is_none() {
                    crate::trace::DeploymentReleaseReason::AssetUnavailable
                } else if obs.my_queued_units.contains(&unit.id) {
                    crate::trace::DeploymentReleaseReason::Preempted
                } else if obs.tick >= deployment.deadline {
                    crate::trace::DeploymentReleaseReason::DeadlineExpired
                } else {
                    crate::trace::DeploymentReleaseReason::PressureCleared
                };
            ...
            if (returning || needs_dispatch) && !routes.group_reaches_command_goal(&[unit.id], goal)
            {
                ...
                self.state.support_deployments.released.push((
                    deployment,
                    crate::trace::DeploymentReleaseReason::RouteUnavailable,
                ));
                continue;
            }
            if needs_dispatch {
    ...
    pub(crate) fn prepare_support_deployments(
        &self,
        context: EconomicInvestmentContext<'_>,
        tuning: DifficultyTuning,
        minimum_core: u32,
