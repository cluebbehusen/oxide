...
impl UtilityPolicy {
    ...
    pub(super) fn economy(
        ...
        let mut routes = danger.as_ref().map(|danger| {
            crate::navigation::commands::RouteProjection::ground_avoiding(
    ...
    pub(super) fn source_in_salvage_incident(obs: &Observation, source: TilePos) -> bool {
        obs.salvage_incidents.iter().any(|incident| {
            incident.chebyshev(source) <= oxide_sim::stats::HARVEST_INCIDENT_DANGER_RADIUS
        })
    }
    ...
    fn harvester_reaches_drop_off(
        obs: &Observation,
        harvester: &UnitObs,
        routes: &mut crate::navigation::commands::RouteProjection<'_>,
    ) -> bool {
        obs.my_buildings
            .iter()
    ...
    pub(super) fn production_with_air_demand(
        ...
        self.residual_production(
            ...
            context,
            budget,
            FoundryHandoff::default(),
            intents,
        )
    }

    pub(super) fn residual_production(
        &mut self,
        dials: &Dials,
        obs: &Observation,
        context: ProductionContext<'_>,
    ...
    pub(super) fn ordinary_ground_has_work(&self, obs: &Observation, home: TilePos) -> bool {
        ...
        let mut routes = crate::navigation::commands::RouteProjection::known_ground(
            QueryPurpose::HarvestDispatch,
            obs,
        );
        if obs
...
mod tests {
    ...
    use crate::intelligence::{ContactEvidence, StrategicIntelligence};
    use crate::observation::BuildingObs;
    use oxide_sim::command::Command;
    use oxide_sim::ids::{BuildingId, PlayerId};
    use oxide_sim::scenario::{BotConfig, BotDifficulty, BotStance, PlayerSpec, Scenario};
    use oxide_sim::state::Faction;

    fn observation() -> Observation {
        let harvester =
            crate::test_support::unit(3, PlayerId(0), UnitKind::Harvester, TilePos::new(3, 4));
        Observation::from_data(ObservationData {
            tick: 0,
            map_width: 16,
            map_height: 10,
            my_units: vec![harvester],
            my_buildings: vec![crate::test_support::building(
                0,
                PlayerId(0),
    ...
    fn direct_player_facing_economy(
        ...
        let enemy_base = obs
            ...
            .min()
            .map(|(_, y, x)| TilePos::new(x, y));
        let mut routes = crate::navigation::commands::RouteProjection::ground_avoiding(
    ...
    fn add_unit(obs: &mut Observation, id: u32, kind: UnitKind, tile: TilePos) {
        obs.my_units
            .push(crate::test_support::unit(id, PlayerId(0), kind, tile));
    }
    ...
    fn player_facing_residual_production_does_not_duplicate_standing_combat_demand() {
        ...
        assert!(
            intents.iter().all(|intent| !matches!(
                intent,
                Intent::TrainAt { kind, .. } if kind.stats().can_fight()
            )),
            "standing combat purchases belong to the shared allocation session: {intents:?}"
        );
    }
    ...
    #[test]
    fn unfinished_security_becomes_a_fresh_foundry_preparation_need() {
        let home = TilePos::new(1, 1);
        ...
        assert_eq!(
            ...
            "shared allocation must retain the exact missing protection demand"
        );
    }
    ...
    fn remembered_danger_releases_an_unsafe_foundry_reserve_to_core_production() {
        ...
        assert!(
            ...
            "released capital must wait for the shared standing-force allocator: {intents:?}"
        );
        assert_eq!(budget, threatened.scrap);
    }
    ...
    fn allocate_then_residual(
        ...
        let outcome = crate::allocation::test_allocate_policy(policy, dials, obs, map, enlisted);
        assert!(
            outcome.allocation_ok,
            "fixture allocation must settle at tick {}",
            obs.tick
        );
        let mut intents = outcome.fresh_emergency_defense_intents;
        intents.extend(outcome.fresh_foundry_intents);
        intents.extend(outcome.maintenance_intents);
        intents.extend(outcome.fresh_economy_intents);
        intents.extend(outcome.fresh_defense_intents);
        intents.extend(outcome.allocated_producer_intents);
        let mut core_exclusions = policy.support_reservations();
        core_exclusions.extend(policy.state.reconnaissance.reservations());
        let context = StrategicUtilityContext::new(
            &outcome.planner_claims,
            &[],
            &[],
            map,
            intents,
        ...
        .with_combat_core_exclusions(&core_exclusions)
        .with_prior_scrap_commitment(obs.scrap.saturating_sub(outcome.budget.utility_spendable))
        .with_foundry_handoff(outcome.foundry_handoff)
        .with_voluntary_scrap_guard(outcome.budget.voluntary_scrap_guard)
        .with_producer_lane_reservations(&outcome.producer_lane_reservations);
        policy.think_with_intelligence(dials, obs, &[], enlisted, context)
    }
    ...
    fn saved_foundry_plan_releases_on_transition_or_invalidation() {
        ...
        assert_eq!(control.state.foundry_saving.as_ref(), Some(&saving));
        ...
        for (reason, mut candidate, observation) in [
            ...
            assert!(candidate.state.foundry_saving.is_none(), "{reason}");
            assert!(
    ...
    #[test]
    fn conflicting_saved_foundry_ownership_freezes_allocation_without_partial_dispatch() {
        let fixture = saved_foundry_fixture();
        let (mut policy, saving, _) = begin_foundry_saving(&fixture);
        let before = policy.state.foundry_saving.clone();
        let outcome = crate::allocation::test_allocate_policy(
            &mut policy,
            &fixture.dials,
            &fixture.obs,
            &fixture.public_map,
    ...
    fn older_deferred_construction_keeps_priority_over_foundry_saving() {
        ...
        assert_eq!(
            observation
                ...
                .unwrap()
                .founding,
            Some((BuildingKind::Fabricator, deferred_anchor))
        );
    }
    #[test]
    fn funded_foundry_commitment_emits_exact_build_then_releases_on_dispatch() {
        ...
        funded.tick = crate::difficulty::next_strategic_admission_tick(fixture.obs.tick);
        funded.scrap = saving.required_scrap;
        let mut funded_intents = allocate_then_residual(
            &mut policy,
            &fixture.dials,
            &funded,
            &fixture.public_map,
    ...
    fn same_think_adjudicated_foundry_build_is_not_rederived_as_a_blocked_saving() {
        ...
        let mut funded = fixture.obs.clone();
        funded.scrap = saving.required_scrap;
        let mut budget = funded.scrap.saturating_sub(saving.required_scrap);
        ...
        let promised = policy.residual_production(
            &fixture.dials,
            &funded,
    ...
    #[test]
    fn residual_foundry_safety_resets_recovery_without_dispatching_new_current_capital() {
        let fixture = saved_foundry_fixture();
        let (mut policy, saving, _) = begin_foundry_saving(&fixture);
        let mut funded = fixture.obs.clone();
        funded.tick = crate::difficulty::next_strategic_admission_tick(funded.tick);
        funded.scrap = saving.required_scrap;
        policy.state.foundry_saving.as_mut().unwrap().blocked_since = Some(fixture.obs.tick);
        let mut handoff = policy.foundry_handoff();
        handoff.current_scrap = funded.scrap;
        let mut budget = handoff.spending_after(funded.scrap);
        let mut intents = Vec::new();
        assert!(
        ...
        assert_eq!(policy.state.foundry_saving.as_ref(), Some(&saving));
        assert_eq!(budget, 0);
    }
    ...
    fn same_think_forecast_funded_foundry_save_survives_current_only_utility() {
        ...
        let mut budget = observation.scrap.saturating_sub(accepted.required_scrap);
        ...
        assert!(promised);
        assert!(intents.is_empty());
        assert_eq!(policy.state.foundry_saving.as_ref(), Some(&accepted));
        assert_eq!(budget, 0);
    }
    ...
    fn forecast_funded_foundry_keeps_its_fixed_horizon_until_income_disappears() {
        ...
        let extractor = accepted_observation
            .my_buildings
            .iter()
            .position(|building| building.id == BuildingId(11))
            .expect("the fixture has its remote Extractor");
        accepted_observation.my_buildings.remove(extractor);
        accepted_observation.my_queues.remove(extractor);
        accepted_observation.known_scrap = vec![(TilePos::new(30, 16), 800)];
        let foundry_cost = BuildingKind::Foundry
            .base_stats()
            .construction
        ...
        let non_reclaimer_forecast = ResourceSnapshot::from_observation(&accepted_observation)
            ...
            .amount();
        let protected_reserve = 0;
        let required_scrap = foundry_cost;
        assert!(
        ...
        assert!(
            ResourceSnapshot::from_observation(&accepted_observation)
                ...
                .amount()
                >= required_scrap,
            "the added completed source must honestly cover the accepted quote"
        );
        ...
        let proposal = expect_ready_foundry(
            ...
            "the completed-source forecast should support one ready exact expansion",
        );
        ...
        let advance = |policy: &mut UtilityPolicy, observation: &Observation| {
            let intents = allocate_then_residual(
                policy,
                &fixture.dials,
                observation,
                &fixture.public_map,
        ...
        shortly_after_acceptance.tick = fixture.dials.cadence;
        ...
        intact.tick = FOUNDRY_RECOVERY_TICKS.saturating_add(fixture.dials.cadence * 2);
    ...
    fn current_funded_foundry_build_can_coexist_with_free_mobile_support() {
        ...
        let proposal = expect_ready_foundry(
            ...
            "the current bank should support the exact ready expansion",
        );
        assert_eq!(
            proposal.adjudicated_commit(),
        ...
        policy.admit_test_repair(
            ...
            &fixture.public_map,
            UnitId(101),
            oxide_sim::ids::Target::Unit(UnitId(102)),
            &mut prelude,
        );
    ...
    fn mirrored_foundry_builder_lease_lowers_the_exact_world_anchor() {
        ...
        funded.tick = crate::difficulty::next_strategic_admission_tick(funded.tick);
        funded.scrap = saving.required_scrap;
        let mut intents = allocate_then_residual(
            &mut policy,
            &fixture.dials,
            &funded,
            &fixture.public_map,
    ...
    #[test]
    fn retained_foundry_identity_survives_compatible_current_investments() {
        let fixture = saved_foundry_fixture();
        let (mut policy, saving, _) = begin_foundry_saving(&fixture);
        let mut underfunded = fixture.obs.clone();
        let cadence = crate::difficulty::next_strategic_admission_tick(underfunded.tick);
        ...
        for tick in [cadence, cadence.saturating_mul(2)] {
            ...
            assert!(!intents.iter().any(|intent| matches!(intent,
                Intent::BuildWith { builder, .. } if *builder == saving.plan.builder)));
        }
    }
    ...
    fn saved_foundry_keeps_its_adjudicated_safety_guard() {
        ...
        guarded.tick = crate::difficulty::next_strategic_admission_tick(guarded.tick);
    ...
    fn saved_foundry_releases_its_hold_for_newly_required_preparation() {
        ...
        underprotected.tick = crate::difficulty::next_strategic_admission_tick(underprotected.tick);
        underprotected
            .my_units
        ...
        let mut handoff = policy.foundry_handoff();
        handoff.current_scrap = underprotected.scrap;
        let mut budget = handoff.spending_after(underprotected.scrap);
        let mut intents = Vec::new();
        let claims = ConstructionClaims {
            cancellations: FoundationCancellations::default(),
            enlisted: &[],
        ...
        assert_eq!(policy.state.foundry_saving, None);
        assert_eq!(policy.foundry_handoff(), FoundryHandoff::default());
    }
    ...
    fn blocked_foundry_saving_releases_after_the_bounded_recovery_window() {
        ...
        blocked.enemy_units.push(crate::test_support::unit(
            900,
            PlayerId(1),
            UnitKind::Sentinel,
            saving.plan.anchor,
        ));
        let first_blocked = crate::difficulty::next_strategic_admission_tick(fixture.obs.tick);

        blocked.tick = first_blocked;
        let _ = allocate_then_residual(
            &mut policy,
            &fixture.dials,
            &blocked,
            &fixture.public_map,
        ...
        assert!(
            policy.state.foundry_saving.is_none(),
    ...
    fn recovered_foundry_saving_gets_a_fresh_bounded_recovery_window() {
        ...
        recovered.tick = crate::difficulty::next_strategic_admission_tick(first_blocked);
        let _ = allocate_then_residual(
            &mut policy,
            &fixture.dials,
            &recovered,
        ...
        let second_blocked = crate::difficulty::next_strategic_admission_tick(recovered.tick);
    ...
    #[test]
    fn occupied_saved_foundry_site_releases_after_continuous_blockage() {
        let fixture = saved_foundry_fixture();
        let (mut policy, saving, _) = begin_foundry_saving(&fixture);
        let mut blocked = fixture.obs.clone();
        blocked.tick = crate::difficulty::next_strategic_admission_tick(blocked.tick);
        blocked.known_scrap.push((saving.plan.anchor, 1));
        let first = blocked.tick;
        for tick in [first, first + FOUNDRY_RECOVERY_TICKS - 12] {
            blocked.tick = tick;
            let intents = allocate_then_residual(
                &mut policy,
                &fixture.dials,
            ...
            let retained = policy.state.foundry_saving.as_ref().unwrap();
            assert_eq!(retained.plan, saving.plan);
            assert_eq!(retained.blocked_since, Some(first));
            assert!(!intents.iter().any(|intent| matches!(intent, Intent::BuildWith { kind: BuildingKind::Foundry, anchor, .. } if *anchor == saving.plan.anchor)));
        }
        blocked.tick = first + FOUNDRY_RECOVERY_TICKS;
        let _ = allocate_then_residual(
            &mut policy,
            &fixture.dials,
            &blocked,
        ...
        assert!(
            policy
                ...
                .foundry_saving
                .as_ref()
                .is_none_or(|current| current.accepted_at != saving.accepted_at)
        );
    }

    #[test]
    fn cleared_foundry_site_dispatches_the_original_plan() {
        let fixture = saved_foundry_fixture();
        let (mut policy, saving, _) = begin_foundry_saving(&fixture);
        let mut observation = fixture.obs.clone();
        observation.tick = crate::difficulty::next_strategic_admission_tick(observation.tick);
        observation.known_scrap.push((saving.plan.anchor, 1));
        let _ = allocate_then_residual(
            &mut policy,
            &fixture.dials,
            &observation,
        ...
        observation
            .known_scrap
            .retain(|(tile, _)| *tile != saving.plan.anchor);
        observation.scrap = saving.required_scrap;
        let intents = allocate_then_residual(
            &mut policy,
            &fixture.dials,
        ...
        assert_eq!(
            policy.state.foundry_saving.as_ref().unwrap().plan,
            saving.plan
        );
        assert_eq!(
            policy.state.foundry_saving.as_ref().unwrap().blocked_since,
        ...
        assert_eq!(intents.iter().filter(|intent| matches!(intent, Intent::BuildWith { kind: BuildingKind::Foundry, builder, anchor } if *builder == saving.plan.builder && *anchor == saving.plan.anchor)).count(), 1);
    }
    ...
    fn opening_core_loss_releases_saved_foundry_capital_for_reinforcement() {
        ...
        deficient.tick = crate::difficulty::next_strategic_admission_tick(fixture.obs.tick);
        deficient
            .my_units
    ...
    fn ordinary_nearby_losses_do_not_turn_one_warning_into_an_economy_quarantine() {
        ...
        obs.enemy_units.push(crate::test_support::unit(
            38,
            PlayerId(1),
            UnitKind::Gnat,
            TilePos::new(10, 9),
    ...
    fn a_destroyed_worker_is_matched_to_its_active_source_beyond_its_last_tile() {
        ...
        assert!(
            last_seen.chebyshev(source) > oxide_sim::stats::HARVEST_INCIDENT_DANGER_RADIUS,
        ...
        assert_eq!(
            policy.state.contested_harvest_regions,
    ...
    fn upgraded_turret_range_marks_a_source_as_known_danger() {
        ...
        obs.enemy_buildings.push(BuildingObs {
            hp: BuildingKind::Turret.tier_stats(1).max_hp,
            tier: 1,
            ..crate::test_support::building(9, PlayerId(1), BuildingKind::Turret, anchor)
        });
    ...
    fn visible_mobile_threat_redirects_work_until_it_leaves() {
        ...
        obs.enemy_units.push(UnitObs {
            idle: false,
            ..crate::test_support::unit(9, PlayerId(1), UnitKind::Sentinel, threatened.offset(3, 0))
        });
        let mut policy = UtilityPolicy::new();
        let mut intents = Vec::new();
    ...
    fn remembered_mobile_threat_survives_the_sight_collapse_after_a_worker_loss() {
        ...
        obs.enemy_units.push(UnitObs {
            idle: false,
            ..crate::test_support::unit(9, PlayerId(1), UnitKind::Sentinel, last_seen)
        });
    ...
    fn active_harvesters_and_builders_evacuate_once_then_retry_only_if_stuck() {
        ...
        assert!(!policy.state.evacuating_workers.contains(&UnitId(3)));
    }
