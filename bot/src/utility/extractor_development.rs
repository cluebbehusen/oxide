    ...
    pub(super) fn value_extractor_developments<'a>(
        ...
        let geometry = geometry.get_or_insert_with(|| {
            ConstructionChecks::new(
                crate::query_work::QueryPurpose::ExtractorCluster,
                self,
                obs,
                context.briefing,
        ...
        for (members, mut anchors) in sites {
            ...
            if !existing {
                ...
                let support_return =
                    horizon.saturating_sub(elapsed) * income
                        / (u64::from(oxide_sim::TICKS_PER_SECOND) * 60)
                        + horizon.saturating_sub(elapsed.max(
                            oxide_sim::stats::FOUNDRY_DRIP_START_TICK.saturating_sub(obs.tick),
...
fn compact_group(frames: impl Iterator<Item = TilePos>) -> bool {
    ...
    bounds.is_some_and(|(left, right, top, bottom)| {
        right - left <= oxide_sim::stats::EXTRACTOR_SUPPORT_RADIUS
            && bottom - top <= oxide_sim::stats::EXTRACTOR_SUPPORT_RADIUS
    })
}
