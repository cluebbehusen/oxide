...
use oxide_sim::stats::{BuildingKind, Domain, Role, UnitKind};
...
impl StandingForceProposal {
    ...
    pub(crate) const fn reason(&self) -> StandingForceReason {
        self.reason
    }
...
fn force_projection_candidates(
    ...
    let (line_weight, siege_weight) = match profile.stance {
        oxide_sim::scenario::BotStance::Turtle => (120, 20),
        oxide_sim::scenario::BotStance::Balanced => (100, 30),
        oxide_sim::scenario::BotStance::Aggressive => (80, 50),
    };
...
mod tests {
    ...
    fn exact_active_worker_repair_does_not_buy_a_duplicate_tender() {
        ...
        obs.my_repair_targets = vec![(UnitId(80), oxide_sim::ids::Target::Unit(patient))];
        let (profile, tuning) = prime();
        assert!(
            derive_all(
                &obs,
