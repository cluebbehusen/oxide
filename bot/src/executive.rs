...
//!
//! The utility policy never touches [`oxide_sim::PlayerCommand`] directly. It
...
pub(super) fn weapon_burst_dps100(w: &oxide_sim::stats::WeaponStats) -> u64 {
...
mod tests {
    ...
    fn a_stranded_economy_reserves_its_bank_for_exactly_one_harvester() {
        ...
        obs.my_queues[foundry_index] = vec![UnitKind::Sentinel; oxide_sim::stats::QUEUE_CAP];
        assert_eq!(
            exec.harvester_recovery(PlayerId(0), &obs),
            Some(Vec::new()),
