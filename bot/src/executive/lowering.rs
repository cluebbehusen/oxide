            ...
            && let Some((_, foundry)) = obs
                ...
                .filter(|(qi, b)| {
                    b.kind == BuildingKind::Foundry
                        && b.built
                        && obs.my_queues[*qi].len() < oxide_sim::stats::QUEUE_CAP
                })
                .min_by_key(|(_, b)| b.id)
        {
            ...
            .filter(|unit| {
                !require_route
                    || match proposed_footprint {
                        None => routes
                            .get_or_insert_with(|| {
                                crate::navigation::commands::RouteProjection::new(QueryPurpose::ExecutiveOrders,
                                    obs,
                                    oxide_sim::stats::Domain::Ground,
                                )
                            })
                            .group_reaches_command_goal(&[unit.id], anchor),
                        Some(size) => crate::navigation::commands::unit_reaches_build_site_via(
                            routes.get_or_insert_with(|| {
                                crate::navigation::commands::RouteProjection::ground_excluding_footprint(QueryPurpose::ExecutiveOrders,
                                    obs, anchor, size,
                                )
                            }),
                            unit,
    ...
    fn draft(
        ...
        let mut candidates: Vec<(i32, UnitId)> = obs
            ...
            .filter(|u| {
                ...
                stats.can_fight()
                    && stats.domain == oxide_sim::stats::Domain::Ground
                    && u.idle
                    && !enlisted.contains(&u.id)
                    && !claimed.contains(&u.id)
                    && !reserved.contains(&u.id)
        ...
        let routes = crate::navigation::commands::RouteProjection::known_ground(
...
mod tests {
    use super::*;
    use crate::observation::{BuildingObs, OBSERVATION_VERSION};
    ...
    fn concurrent_ground_bodies_share_one_physical_objective_credit() {
        use crate::experience::{Doctrine, Experience};
        let (mut obs, mut executive) = target_holding_position();
        let mut second = executive.armies[0].clone();
        second.id = ArmyId(8);
        second.members = (100..=104).map(UnitId).collect();
    ...
    fn foundry_builder_lease_rejects_wrong_build_and_implicit_worker_theft() {
        ...
        obs.my_buildings = vec![crate::observation::BuildingObs {
            hp: 1,
            ..crate::test_support::building(
                9,
                PlayerId(0),
