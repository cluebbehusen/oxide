...
pub(crate) fn building_threatens(
    obs: &Observation,
...
pub(crate) fn building_strength(building: &BuildingObs, domain: Domain) -> u64 {
    if !building.built {
        return 0;
    }
...
mod tests {
    ...
    fn direct_fire_crosses_known_pits_but_not_rock_or_peaks() {
        for terrain in [
            oxide_sim::map::Terrain::Pit,
            oxide_sim::map::Terrain::Rock,
            oxide_sim::map::Terrain::Peak,
        ] {
            let blocker = TilePos::new(6, 4);
            let mut obs = Observation::default();
            obs.known_rock.push(blocker);
            if terrain == oxide_sim::map::Terrain::Pit {
                obs.known_pits.push(blocker);
            }
            if terrain == oxide_sim::map::Terrain::Peak {
