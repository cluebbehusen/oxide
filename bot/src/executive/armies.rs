        ...
        for army in &mut self.armies {
            ...
            let unexplained_loss = self
                ...
                .is_some_and(|journal| {
                        ...
                        && journal.observe_objective(obs, target.observed_id(obs))
                    {
                        journal.finish(
                            obs,
                            crate::experience::Outcome::Complete,
                            crate::experience::OutcomeReason::ObjectiveObservedGone,
                            750,
                            false,
                        );
                    }
                ...
                && unexplained_loss
            {
                ...
                self.ground_outcomes
                    .get_mut(&army.id)
                    .expect("observed casualty journal")
                    .finish(
                        obs,
                        crate::experience::Outcome::Inconclusive,
                        crate::experience::OutcomeReason::LostContact,
                        0,
                        false,
                    );
                out.push(PlayerCommand {
                ...
                && centroid.chebyshev(mission.goal) > 2
            {
                    ...
                    && !tiles_within(centroid, army.staging, 2)
                {
                    ...
                    let bounced = members.iter().all(|unit| unit.idle)
                        && army.issued.is_some_and(|(tick, from)| {
                            obs.tick > tick && centroid.chebyshev(from) <= 1
                        });
                        ...
                        && (!tiles_within(centroid, army.staging, 2))
                    {
                        ...
                        let legal_focus = |unit: &UnitObs| {
                            ...
                            obs.visible(unit.tile)
                                ...
                                && unit.body_domain() == oxide_sim::stats::Domain::Ground
                                ...
                                if let Some(target) = focus {
                                    ...
                                    out.push(PlayerCommand {
                                        ...
                                        command: Command::Attack {
                                            units,
                                            target: oxide_sim::ids::Target::Unit(target).into(),
                                            queue: false,
                                        },
                                    });
...
pub(super) fn is_artillery(u: &UnitObs) -> bool {
    ...
    stats
        .max_range_vs(oxide_sim::stats::Domain::Ground)
        .is_some_and(|r| r > chassis::fx::Fx::from_num(stats.vision))
}

fn can_attack_domain(unit: &UnitObs, domain: oxide_sim::stats::Domain) -> bool {
    let stats = unit.kind.stats();
    stats.can_target(domain) || (stats.demolition && domain == oxide_sim::stats::Domain::Ground)
}
...
fn can_focus_without_chasing(attacker: &UnitObs, target: &UnitObs) -> bool {
    ...
    attacker.kind.stats().weapons.iter().any(|weapon| {
        ...
        && target_domain == oxide_sim::stats::Domain::Ground
        && distance
            <= oxide_sim::stats::SAPPER_CONTACT_RANGE
                .ceil()
                .to_num::<i32>())
}
...
fn local_fight_strength(obs: &Observation, members: &[&UnitObs]) -> (u64, u64) {
    let defenses: Vec<_> = obs
        ...
        .collect();
    let engaged: Vec<_> = members
        .iter()
        .copied()
        .filter(|member| {
            obs.enemy_units.iter().any(|enemy| {
                mutually_relevant(member, enemy)
                    && member.tile.chebyshev(enemy.tile) <= CONTACT_RADIUS
            }) || defenses.iter().any(|building| {
                super::threats::building_threatens(obs, building, member.tile, member.body_domain())
            })
        })
        .collect();
    let nearby: Vec<_> = members
        .iter()
        .copied()
        .filter(|member| {
            engaged
                .iter()
                .any(|front| member.tile.chebyshev(front.tile) <= ENGAGE_RADIUS)
        })
        .collect();
    let opposition: Vec<_> = obs
        ...
        .filter(|enemy| {
            engaged
                .iter()
                .any(|front| front.tile.chebyshev(enemy.tile) <= ENGAGE_RADIUS)
        })
        .collect();
    let (mut mine, mut theirs) = matched_strength(&nearby, &opposition);
    if !defenses.is_empty() {
        mine = nearby
            .iter()
            .map(|unit| {
                strength_vs(unit, oxide_sim::stats::Domain::Ground).max(
                    if opposition
                        .iter()
                        .any(|enemy| enemy.body_domain() == oxide_sim::stats::Domain::Air)
                    {
                        strength_vs(unit, oxide_sim::stats::Domain::Air)
                    } else {
                        0
                    },
                )
            ...
            .sum();
        theirs = theirs.saturating_add(
            defenses
                .iter()
                .map(|building| {
                    super::threats::building_strength(building, oxide_sim::stats::Domain::Ground)
                })
                .sum::<u64>(),
        );
    }
...
pub(crate) fn locally_overmatches_near(
    obs: &Observation,
    members: &[UnitId],
    target: TilePos,
    radius: i32,
...
mod tests {
    ...
    #[test]
    fn siege_keeps_its_screen_with_the_guns_and_holds_while_firing() {
        let staging = TilePos::new(4, 10);
        let target = TilePos::new(26, 10);
        let units = vec![
            unit(
                1,
                PlayerId(0),
        ...
        let mut obs = observation(120, (40, 24), units, Vec::new());
        obs.enemy_buildings
            .push(building(90, PlayerId(1), BuildingKind::Turret, target));
        let mut body = army(
            1,
    ...
    #[test]
    fn static_fire_counts_as_contact_and_withdrawal_does_not_reacquire_it() {
        ...
        let mut turret = building(90, PlayerId(1), BuildingKind::Turret, TilePos::new(24, 10));
        turret.tier = 2;
        turret.hp = turret.kind.tier_stats(2).max_hp;
        obs.enemy_buildings.push(turret);
        ...
        assert_eq!(
            ...
            crate::experience::OutcomeReason::UnsafeApproach
        );
        obs.tick += 12;
        executive.maintain_player_facing(PlayerId(0), &obs, staging);
    ...
    #[test]
    fn unseen_casualty_prompts_a_return_without_inventing_a_target() {
        let staging = TilePos::new(3, 10);
        let mut obs = observation(
            24,
            (40, 30),
    ...
    #[test]
    fn recover_mission_does_not_reacquire_or_repeat_a_busy_return() {
        let goal = TilePos::new(2, 2);
        let mut obs = observation(
            24,
            (40, 30),
            vec![unit(
        ...
        obs.tick = 96;
        executive.maintain_player_facing(PlayerId(0), &obs, goal);
        assert_eq!(executive.armies[0].state, ArmyState::Engaging);
    }

    #[test]
    fn blocked_recovery_releases_members_without_fabricating_casualties() {
        let from = TilePos::new(14, 10);
        let goal = TilePos::new(2, 2);
        let obs = observation(
            24,
            (40, 30),
            vec![unit(1, PlayerId(0), UnitKind::Sentinel, from, 200, true)],
            Vec::new(),
        );
        let mut executive = Executive::default();
        let mut body = army(0, vec![UnitId(1)], ArmyState::Staging, goal);
    ...
    #[test]
    fn exact_reorganization_is_atomic_and_keeps_both_minimums() {
        let staging = TilePos::new(3, 3);
        let obs = observation(
            24,
            (40, 30),
            (1..=6)
                .map(|id| unit(id, PlayerId(0), UnitKind::Sentinel, staging, 200, true))
                .collect(),
            Vec::new(),
        );
        let mut executive = Executive::default();
        executive.armies.push(army(
            0,
            (1..=6).map(UnitId).collect(),
            ArmyState::Staging,
            staging,
        ));
        executive.next_army = 1;
        ...
        assert_eq!(
            ...
            vec![UnitId(3), UnitId(4), UnitId(5), UnitId(6)]
        );
        assert_eq!(executive.armies[1].members, vec![UnitId(1), UnitId(2)]);
    }

    #[test]
    fn engaged_or_incompatible_staging_bodies_cannot_be_reorganized() {
        let staging = TilePos::new(3, 3);
        let mut obs = observation(
            24,
            (40, 30),
        ...
        assert_eq!(
            ...
            vec![PlayerCommand {
                ...
                command: Command::FocusFire {
                    ...
                    target: oxide_sim::AttackTarget::Unit(UnitId(100)),
                },
            }]
        );
    ...
    fn player_facing_army_completes_a_contact_to_objective_lifecycle() {
        ...
        assert!(
            matches!(
                &commands[0].command,
                Command::Attack { units, target: oxide_sim::AttackTarget::Unit(UnitId(101)), queue: false }
    ...
    fn player_facing_focus_stays_locked_until_the_target_is_no_longer_legal() {
        ...
        assert!(matches!(
            ...
            [PlayerCommand {
                command: Command::Attack {
                    target: oxide_sim::AttackTarget::Unit(UnitId(101)),
                    ..
                },
                ..
            }]
        ...
        for invalidation in ["gone", "hidden", "distant", "untargetable"] {
            ...
            assert!(
                matches!(
                    ...
                    [PlayerCommand {
                        command: Command::Attack {
                            target: oxide_sim::AttackTarget::Unit(UnitId(102)),
                            ..
                        },
                        ..
                    }]
    ...
    fn coordinated_focus_accepts_a_parked_airframe_as_a_ground_body() {
        ...
        assert!(
            !UnitKind::Lancer
                .stats()
                .can_target(oxide_sim::stats::Domain::Air),
    ...
    fn matched_strength_counts_a_parked_airframe_in_the_ground_column() {
        ...
        assert_eq!(
            mine_parked,
            strength_vs(&lancer, oxide_sim::stats::Domain::Ground)
        );
        ...
        assert_eq!(
            parked_vs_ground,
            strength_vs(&condor, oxide_sim::stats::Domain::Ground)
        );
