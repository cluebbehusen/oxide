...
impl Task {
    ...
    fn work_cost(&self) -> usize {
        1 + self.claims.producer_jobs.len() * 2
            + self.capacity.resources.producers().len() * crate::stats::QUEUE_CAP
...
pub(in crate::bot) struct ProductionWork {
    tasks: Vec<Task>,
    next_pending: usize,
}

impl ProductionWork {
    pub(in crate::bot) fn counts(&self) -> (usize, usize) {
        (
    ...
    pub(in crate::bot) fn resume_pending(&mut self, tick: Tick, budget: &mut WorkBudget) {
        self.tasks
...
mod tests {
    use super::*;
    use crate::bot::resources::ResourcePlanningFixture;
    ...
    fn repeated_production_queries_share_the_controllers_remaining_allowance() {
        let (capacity, claims) = fixture(0, 12);
        let work = crate::bot::planning::PlanningWork::with_allowance(1_024);
        for _ in 0..100 {
            assert_eq!(
    ...
    fn constructive_funding_seeks_the_income_boundary_instead_of_enumerating_the_horizon() {
        use crate::bot::resources::ForecastAvailability;
