...
pub(in crate::bot) struct DecisionContext<'a> {
...
pub(in crate::bot) struct AdmittedWork {
...
pub(in crate::bot) struct UtilityGrant {
    pub reservations: Vec<UnitId>,
...
pub(in crate::bot) fn admit_decision(
    ...
    let lift_support_request = lifts
        .operation()
        .filter(|operation| operation.phase <= crate::bot::lift::LiftPhase::AwaitSupport)
...
fn team_channel_state(planner: &TeamReliefPlanner) -> ChannelState {
    if let Some(operation) = planner.operation() {
        let phase = match operation.phase {
            crate::bot::team::TeamReliefPhase::Deploying => ChannelPhase::TeamDeploying,
            crate::bot::team::TeamReliefPhase::Holding => ChannelPhase::TeamHolding,
            crate::bot::team::TeamReliefPhase::Withdrawing => ChannelPhase::TeamWithdrawing,
        };
...
fn lift_channel_state(planner: &LiftPlanner) -> ChannelState {
    ...
    let phase = match operation.phase {
        crate::bot::lift::LiftPhase::Provision => ChannelPhase::LiftProvision,
        crate::bot::lift::LiftPhase::Boarding => ChannelPhase::LiftBoarding,
        crate::bot::lift::LiftPhase::AwaitSupport => ChannelPhase::LiftAwaitSupport,
        crate::bot::lift::LiftPhase::Landing => ChannelPhase::LiftLanding,
        crate::bot::lift::LiftPhase::Recover => ChannelPhase::LiftRecover,
    };
    ChannelState::Active(phase)
}
...
fn raid_channel_state(planner: &RaidPlanner) -> ChannelState {
    if let Some(operation) = planner.operation() {
        let phase = match operation.phase {
            crate::bot::raid::RaidPhase::Ingress => ChannelPhase::RaidIngress,
            crate::bot::raid::RaidPhase::Strike => ChannelPhase::RaidStrike,
            crate::bot::raid::RaidPhase::Egress => ChannelPhase::RaidEgress,
        };
        ChannelState::Active(phase)
...
pub(in crate::bot) fn retained_reservations(
