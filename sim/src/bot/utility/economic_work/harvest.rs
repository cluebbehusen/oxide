...
pub(in crate::bot::utility) struct HarvestRegion {
    pub(in crate::bot::utility) service: TilePos,
    pub(in crate::bot::utility) work: HarvestWork,
    pub(in crate::bot::utility) workers: Vec<WorkerService>,
    producer_access: std::collections::BTreeMap<BuildingId, u32>,
}

impl HarvestRegion {
    pub(in crate::bot::utility) fn current_output(&self, horizon: u64) -> u64 {
        harvest_output(self.work, &self.workers, horizon)
    }

    pub(in crate::bot::utility) fn marginal(&self, worker: WorkerService, horizon: u64) -> u64 {
        marginal_worker_return(self.work, &self.workers, worker, horizon)
    }

    pub(in crate::bot::utility) fn producer_distance(&self, producer: BuildingId) -> Option<u32> {
...
pub(in crate::bot::utility) struct HarvestGeometryCache {
...
impl UtilityPolicy {
    pub(in crate::bot::utility) fn economic_harvest_regions(
        &self,
        obs: &Observation,
        briefing: &PublicMapBriefing,
        resources: &ResourceSnapshot,
...
mod tests {
    ...
    fn harvest_regions_share_dropoffs_without_double_counting_work_or_worker_supply() {
        ...
        let (regions, cold_work) =
            crate::bot::navigation::work::measure(|| evaluate(&obs, &[UnitId(3)]));
