...
at startup without changing authored atlas bytes. Independent regions have
extruded borders; reduction, linear sampling, level blending, and compositing
retain premultiplied alpha. Physical destination size, including DPI and both
axes, selects levels with a fixed -0.4 detail bias. Terrain and unrelated
...
use world-anchored cells and follow the transition slightly later. Marker
visibility shares player exploration, known claims, apparent buildings, and
remembered salvage with world rendering. Ghosts remain distinct from live
observations. Entity positions, selection, picking, and simulation are
unchanged.
