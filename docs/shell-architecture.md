...
presentation caches observe the resulting report. Fast advancement may
suppress intermediate presentation work, but it still bottoms out in the same
recorded tick path.

Presentation may use floats, hash maps, frame time, and interpolation because
it never feeds simulation decisions. Selection is pruned when entities die or
hostiles leave sight; timeline-local aim and effects are cleared after jumps.

## App and screen ownership

`App` owns resources and state that outlive an individual screen: the live
`Game`, configuration, input funnel, scenario draft, tutorial, atlas, sounds,
soundtrack, debug channels, and frame profiler. The `Screen` enum carries the
...
Individual screen modules own their menu cursor, dialogs, draft state, and
windowless update logic. They accept `RawEvent` values and return semantic
transition enums; the `App` coordinator performs session mutations and drawing.
This separation lets the navigation graph and destructive confirmations be
tested without a GPU window.
...
injection appends the same type; `input::apply_events` is the only gameplay
mapper. A behavior that bypasses this funnel is not automatable and violates
the shell contract.
...
but order generation remains gated to the controlled seat. Multi-select and
card actions must preserve set semantics when they become commands.

HUD drawing publishes one `LayoutModel` for the frame. The same rectangles
drive hit testing for the top bar, panel band, order dock, minimap, roster,
...
Disk publication reserves a collision-free destination and delegates the
actual replay write to the chassis atomic-write path. A failed save is a
reported UX outcome rather than a silent quit. Shelf discovery skips malformed
records and labels version-incompatible records instead of trying to guess.
...
re-simulates the suffix; checkpoint cadence stretches to retain at most 64
state clones. The shell slices long seeks across frames, replaces the render
vehicle's state at the destination, and drops effects from the abandoned
timeline.

Resumable records load rather than play fog-free, preventing the replay viewer
from scouting a live match. A completed record may be watched. Final Map is a
camera-only view over the already-final live state, not a replay fast-forward.
...
`DebugSession` and `dispatch_shared` own the common state/clock surface:
status, omniscient state, fog view, state hash, hidden or presented tick
advance, pause/resume, and speed. Each implementation supplies its honest clock
semantics. Requests are capped centrally.

Other requests are capability-specific. Camera, UI, input injection,
screenshots, overlay, and native frame profiling are window-shaped. Commands
and scenario/replay swaps are mutating. The shell answers capabilities its
current screen supports; replay playback refuses mutations; the headless
session refuses window-only requests in words rather than fabricating results.
...
screen chrome. Render interpolation and presentation clocks smooth the fixed
20 Hz simulation without changing its state.

All production sprite regions come from one generated atlas loaded by
`shell/src/assets.rs`. The manifest must match every key the shell resolves,
and the renderer must not load per-sprite textures. Animation state is driven
by simulation events and current actions, then discarded or rebuilt after a
...
whole-map schematic used for deterministic goldens and headless screenshots.
It does not share the GPU renderer's atlas, camera, fog composition, HUD,
animation, or visual polish. Screenshot replies label their producer as
`"gpu"` or `"cpu"` so tests and reviewers cannot compare the wrong surface.
...
|---|---|---|
| App ownership and screen graph | `shell/src/app.rs`, `shell/src/screens/` | `driver/tests/menu_ux.rs`, screen-module unit tests |
...
| GPU assets and rendering | `shell/src/assets.rs`, `shell/src/render.rs` | `shell/tests/presentation_animation.rs`, `driver/tests/native_animation_capture.rs` |
